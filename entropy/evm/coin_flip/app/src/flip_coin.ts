...
async function main() {
  ...
  const flipFee: any = await coinFlipContract.methods.getFlipFee().call();
  console.log(`   fee       : ${flipFee} wei`);

  const receipt: any = await coinFlipContract.methods
  ...
  const intervalId = setInterval(async () => {
    ...
    const event: any = events.find(
      (event: any) => event.returnValues.sequenceNumber === sequenceNumber
    );
