...
import { DataSource } from "../include/imported";
...
export class BaseWrapper implements Contract {
  ...
  static createInitData(config: {
    ...
    const governanceCell = beginCell()
      ...
      .storeUint(config.upgradeCodeHash ?? 0, 256) // upgrade_code_hash, set to 0 for initial state
      .endCell();
