 ...
 * @category loaders
 */
export const fromRecord = migrations => pipe(Object.keys(migrations), Arr.filterMap(_ => Option.fromNullable(_.match(/^(\d+)_(.+)$/))), Arr.map(([key, id, name]) => [Number(id), name, Effect.succeed(migrations[key])]), Arr.sort(migrationOrder), Effect.succeed);
