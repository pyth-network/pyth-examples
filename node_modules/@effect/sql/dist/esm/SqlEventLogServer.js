...
export const makeStorage = options => Effect.gen(function* () {
  ...
  const remoteId = yield* sql`SELECT remote_id FROM ${sql(remoteIdTable)}`.pipe(Effect.flatMap(results => {
    ...
    return Effect.as(sql`INSERT INTO ${sql(remoteIdTable)} (remote_id) VALUES (${newRemoteId})`, RemoteId.make(newRemoteId));
  }));
  ...
  return EventLogServer.Storage.of({
    ...
    write: (publicKey, entries) => Effect.gen(function* () {
      ...
      for (const batch of forInsert) {
        const encryptedEntries = yield* pipe(sql`INSERT INTO ${sql(table)} ${sql.insert(batch.entries)} ON CONFLICT DO NOTHING`.withoutTransform, Effect.zipRight(sql`SELECT * FROM ${sql(table)} WHERE ${sql.in("entry_id", batch.ids)} ORDER BY sequence ASC`), Effect.flatMap(decodeEntries));
...
const EncryptedRemoteEntryFromSql = /*#__PURE__*/Schema.transform(EncryptedRemoteEntrySql, EncryptedRemoteEntry, {
  decode(fromA) {
    return {
 ...
 * @category layers
 */
export const layerStorage = options => Layer.scoped(EventLogServer.Storage, makeStorage(options));
/**
 * @since 1.0.0
 * @category layers
 */
export const layerStorageSubtle = options => layerStorage(options).pipe(Layer.provide(layerSubtle));
