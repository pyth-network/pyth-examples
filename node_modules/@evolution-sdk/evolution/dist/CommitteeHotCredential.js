...
export const FromBytes = /*#__PURE__*/Schema.transformOrFail(Schema.Uint8ArrayFromSelf, Schema.typeSchema(Credential.Credential), {
  ...
  decode: (fromA, _, ast) => Effect.gen(function* () {
    ...
    if (fromA.length !== 29) {
      return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid cc_hot credential length: expected 29 bytes, got ${fromA.length}`));
    }
    ...
    if (keyType !== 0x01) {
      return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid key type in header: expected 0x01 (CC Hot), got 0x0${keyType.toString(16)}`));
    }
    if (credType === 0x0e) {
    ...
    return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid credential type in header: expected 0x0E or 0x0F, got 0x0${credType.toString(16)}`));
  })
}).annotations({
...
//# sourceMappingURL=CommitteeHotCredential.js.map