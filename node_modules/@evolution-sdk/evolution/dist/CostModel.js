 ...
 * ```
 */
export class CostModel extends /*#__PURE__*/Schema.Class("CostModel")({
  costs: /*#__PURE__*/Schema.Array(Schema.BigInt)
}) {
  /**
 ...
 * ```
 */
export class CostModels extends /*#__PURE__*/Schema.Class("CostModels")({
  PlutusV1: CostModel,
  PlutusV2: CostModel,
  PlutusV3: CostModel
}) {
   ...
   */
  [Equal.symbol](that) {
    return that instanceof CostModels && Equal.equals(this.PlutusV1, that.PlutusV1) && Equal.equals(this.PlutusV2, that.PlutusV2) && Equal.equals(this.PlutusV3, that.PlutusV3);
  }
  /**
   * Hash code generation.
...
export const CDDLSchema = /*#__PURE__*/Schema.MapFromSelf({
  key: CBOR.Integer,
  value: /*#__PURE__*/Schema.Array(Schema.BigIntFromSelf)
});
/**
 ...
 */
export const FromCDDL = /*#__PURE__*/Schema.transform(CDDLSchema, /*#__PURE__*/Schema.typeSchema(CostModels), {
  strict: true,
  encode: costModels => {
  ...
  decode: encoded => {
    const v1 = encoded.get(0n);
    const v2 = encoded.get(1n);
    const v3 = encoded.get(2n);
    const result = {
      PlutusV1: new CostModel({
        costs: v1 ?? []
      }),
      PlutusV2: new CostModel({
        costs: v2 ?? []
      }),
      PlutusV3: new CostModel({
        costs: v3 ?? []
      })
    };
 ...
 */
export const arbitrary = /*#__PURE__*/FastCheck.array(FastCheck.bigInt({
  min: 0n,
  max: 1000n
})).map(costs => new CostModel({
  costs
}));
/**
 * CBOR encoding for CostModels.
 */
export const toCBOR = (costModels, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORBytes(options))(costModels);
/**
 ...
 */
export const toCBORHex = (costModels, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORHex(options))(costModels);
/**
 ...
 */
export const languageViewsEncoding = costModels => {
  ...
  for (const [languageId, costs] of languageData.entries()) {
    if (languageId === 0) {
      ...
      const costsArray = costs;
      const indefiniteArrayCbor = CBOR.internalEncodeSync(costsArray, CBOR.CML_DATA_DEFAULT_OPTIONS // indefinite length
      );
