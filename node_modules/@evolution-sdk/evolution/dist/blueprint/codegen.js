...
function toIdentifier(name) {
  ...
  return name.split("/").map(part => part.charAt(0).toUpperCase() + part.slice(1)).join("").replace(/[^a-zA-Z0-9_]/g, "_");
}
/**
 ...
 */
function getNamespacePath(name) {
 ...
 * e.g., "list" -> "List"
 */
function toNamespaceRef(namespacePath) {
 ...
 */
function resolveReference(refName, currentNamespace, config) {
 ...
 */
function inferEncodedTypeString(def, definitions, depth = 0) {
      ...
      case "list":
        {
          ...
          const itemEnc = inferEncodedTypeString(listDef.items, definitions, depth + 1);
 ...
 */
function getEncodedType(refName, definitions) {
 ...
 */
function generateTSchema(def, definitions, config, currentNamespace = "", indent = config.indent, definitionKey, cyclicNames = new Set()) {
      ...
      case "constructor":
        {
          ...
          for (let i = 0; i < constructorDef.fields.length; i++) {
            ...
            const fieldName = field.title || (constructorDef.fields.length === 1 ? config.fieldNaming.singleFieldName : config.fieldNaming.multiFieldPattern.replace("{index}", String(i)));
            let fieldSchema;
            if ("$ref" in field && field.$ref) {
              ...
              fieldSchema = generateTSchema(field.schema, definitions, config, currentNamespace, indent + config.indent + config.indent, undefined, cyclicNames);
            } else {
      ...
      case "list":
        {
          ...
          const itemType = generateTSchema(itemsSchema, definitions, config, currentNamespace, indent, undefined, cyclicNames);
      ...
      case "map":
        {
          ...
          const keyType = generateTSchema(keysSchema, definitions, config, currentNamespace, indent, undefined, cyclicNames);
          const valueType = generateTSchema(valuesSchema, definitions, config, currentNamespace, indent, undefined, cyclicNames);
  ...
  if ("anyOf" in def) {
    const unionDef = def;
    const title = "title" in def ? def.title : undefined;
    // Special transform for Bool type
    if (title === "Bool") {
      const constructors = unionDef.anyOf.filter(item => "dataType" in item && item.dataType === "constructor");
      // Check if it's the standard True/False pattern
      if (constructors.length === 2 && constructors.some(c => c.title === "True") && constructors.some(c => c.title === "False")) {
    ...
    const isVariant = unionDef.anyOf.every(member => {
      if ("dataType" in member && member.dataType === "constructor") {
        const constructorMember = member;
    ...
    if (isVariant) {
      ...
      if (unionDef.anyOf.length === 1) {
        const constructorMember = unionDef.anyOf[0];
      ...
      const typeTitle = "title" in def ? def.title : undefined;
      ...
      if ((useVariant || useExplicitUnion) && !useUnionWithLiterals) {
        ...
        const variantFields = []; // for Variant output
        const explicitUnionMembers = []; // for Union output
        for (const member of unionDef.anyOf) {
          ...
          for (let i = 0; i < fields.length; i++) {
            ...
            if (field.title) {
              ...
              if (customFieldNames && customFieldNames[i]) {
                ...
                fieldName = fields.length === 1 ? config.fieldNaming.singleFieldName : config.fieldNaming.multiFieldPattern.replace("{index}", i.toString());
              }
            }
            let fieldSchema;
            if (field.$ref) {
              ...
              fieldSchema = generateTSchema(field.schema, definitions, config, currentNamespace, indent + config.indent + config.indent + config.indent, undefined, cyclicNames);
            } else {
              fieldSchema = "PlutusData";
            }
          ...
          if (useVariant) {
            ...
            const innerFieldsStr = fieldSchemas.length > 0 ? `{\n${fieldSchemas.map(f => `${indent}${config.indent}${config.indent}${config.indent}${f}`).join(",\n")}\n${indent}${config.indent}${config.indent}}` : "{}";
            explicitUnionMembers.push(`${indent}${config.indent}TSchema.Struct({ ${tag}: TSchema.Struct(${innerFieldsStr}, { flatFields: true }) }, { flatInUnion: true })`);
          }
        }
        if (useVariant) {
          ...
          for (let i = 0; i < fields.length; i++) {
            ...
            if (field.title) {
              ...
              if (customFieldNames && customFieldNames[i]) {
                ...
                fieldName = fields.length === 1 ? tag.charAt(0).toLowerCase() + tag.slice(1) : config.fieldNaming.multiFieldPattern.replace("{index}", i.toString());
              }
            }
            let fieldSchema;
            if (field.$ref) {
        ...
        for (const member of unionDef.anyOf) {
          ...
          taggedStructs.push(`${indent}  TSchema.TaggedStruct("${tag}", {\n${fieldSchemas.join(",\n")}\n${indent}  }, { flatFields: true })`);
        }
        return `TSchema.Union(\n${taggedStructs.join(",\n")}\n${indent})`;
      }
    }
    // Otherwise use regular Union
    const members = def.anyOf.map(memberDef => generateTSchema(memberDef, definitions, config, currentNamespace, indent, undefined, cyclicNames));
    return `TSchema.Union(\n${indent}  ${members.join(`,\n${indent}  `)}\n${indent})`;
  }
  // Handle empty schema
 ...
 */
function extractDependencies(def, _definitions) {
  const deps = new Set();
  function visit(node) {
 ...
 */
function generateTSType(def, definitions, currentNamespace, config, visitedRefs = new Set()) {
  if ("$ref" in def) {
    const refPath = def.$ref.replace("#/definitions/", "");
    const refName = refPath.replace(/~1/g, "/").replace(/~0/g, "~");
  ...
  if ("dataType" in def) {
    switch (def.dataType) {
      ...
      case "list":
        {
          const listDef = def;
          const itemType = listDef.items ? generateTSType(listDef.items, definitions, currentNamespace, config, visitedRefs) : "unknown";
          return `ReadonlyArray<${itemType}>`;
        }
      case "map":
        {
          const mapDef = def;
          const keyType = mapDef.keys ? generateTSType(mapDef.keys, definitions, currentNamespace, config, visitedRefs) : "unknown";
          const valType = mapDef.values ? generateTSType(mapDef.values, definitions, currentNamespace, config, visitedRefs) : "unknown";
      ...
      case "constructor":
        {
          ...
          const fieldStrings = ctorDef.fields.map(f => {
            const fname = f.title ?? "field";
            let ftype;
            if ("$ref" in f) {
              ftype = generateTSType(f, definitions, currentNamespace, config, visitedRefs);
            } else if ("schema" in f && f.schema) {
              ftype = generateTSType(f.schema, definitions, currentNamespace, config, visitedRefs);
            } else {
              ftype = "unknown";
            }
 ...
 */
function topologicalSort(definitions) {
  const sorted = [];
 ...
 */
export function generateTypeScript(blueprint, config = DEFAULT_CODEGEN_CONFIG) {
  const lines = [];
  // Topologically sort definitions to ensure dependencies come first
  // (must happen before imports so we know if cyclic types exist)
  const {
    cyclicNames,
    sorted: sortedDefinitions
  } = topologicalSort(blueprint.definitions);
  // File header
  lines.push("/**");
  ...
  if (config.moduleStrategy === "namespaced") {
    ...
    for (const [fullName, typeDef] of namespacedTypes) {
      ...
      const schemaDefinition = generateTSchema(typeDef, blueprint.definitions, config, ns, nsIndent + (isCyclic ? config.indent : ""), typeName, cyclicNames);
    ...
    for (const [name, def] of sortedDefinitions) {
      ...
      const schemaDefinition = generateTSchema(def, blueprint.definitions, config, "", isCyclic ? config.indent : "", schemaName, cyclicNames);
      // Add JSDoc comment
      if ("title" in def && def.title) {
        lines.push("/**");
        lines.push(` * ${def.title}`);
  ...
  for (const validator of blueprint.validators) {
    ...
    if (validator.redeemer) {
      const redeemerSchema = Object.keys(validator.redeemer.schema).length === 0 ? "PlutusData" : generateTSchema(validator.redeemer.schema, blueprint.definitions, config, "", "  ");
