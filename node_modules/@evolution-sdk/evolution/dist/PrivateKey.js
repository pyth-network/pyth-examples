 ...
 * @category schemas
 */
export class PrivateKey extends /*#__PURE__*/Schema.TaggedClass()("PrivateKey", {
  key: /*#__PURE__*/Schema.Union(Bytes64.BytesFromHex, Bytes32.BytesFromHex)
}) {
  toJSON() {
    return {
      _tag: "PrivateKey",
      key: Bytes.toHex(this.key)
    };
  }
  toString() {
...
export const FromBytes = /*#__PURE__*/Schema.transform(Schema.typeSchema(Schema.Union(Bytes64.BytesFromHex, Bytes32.BytesFromHex)), Schema.typeSchema(PrivateKey), {
  strict: true,
  decode: bytes => new PrivateKey({
    key: bytes
  }),
...
export const FromHex = /*#__PURE__*/Schema.compose(Schema.Uint8ArrayFromHex,
// string -> Uint8Array (any length)
FromBytes // Uint8Array -> PrivateKey (validates 32 or 64)
).annotations({
  identifier: "PrivateKey.FromHex"
});
export const FromBech32 = /*#__PURE__*/Schema.transformOrFail(Schema.String, Schema.typeSchema(PrivateKey), {
  strict: true,
  encode: (_, __, ___, toA) => E.gen(function* () {
  ...
  decode: (fromA, _, ast) => E.gen(function* () {
    const {
      prefix,
      words
    } = yield* ParseResult.try({
      ...
      try: () => bech32.decode(fromA, 1023),
      catch: error => new ParseResult.Type(ast, fromA, `Failed to decode bech32 string: ${error.message}`)
    });
 ...
 * @category arbitrary
 */
export const arbitrary = /*#__PURE__*/FastCheck.uint8Array({
  minLength: 32,
  maxLength: 32
}).map(bytes => new PrivateKey({
  key: bytes
}, {
 ...
 * @category parsing
 */
export const fromBytes = /*#__PURE__*/Schema.decodeSync(FromBytes);
/**
 ...
 * @category parsing
 */
export const fromHex = /*#__PURE__*/Schema.decodeSync(FromHex);
/**
 ...
 * @category parsing
 */
export const fromBech32 = /*#__PURE__*/Schema.decodeSync(FromBech32);
// ============================================================================
// Encoding Functions
// ============================================================================
/**
 ...
 * @category encoding
 */
export const toBytes = /*#__PURE__*/Schema.encodeSync(FromBytes);
/**
 ...
 * @category encoding
 */
export const toHex = /*#__PURE__*/Schema.encodeSync(FromHex);
/**
 ...
 * @category encoding
 */
export const toBech32 = /*#__PURE__*/Schema.encodeSync(FromBech32);
// ============================================================================
// Factory Functions
// ============================================================================
/**
 ...
 */
export const toPublicKey = privateKey => VKey.fromPrivateKey(privateKey);
/**
 ...
 * @category bip39
 */
export const generateMnemonic = (strength = 256) => BIP39.generateMnemonic(wordlist, strength);
/**
 ...
 * @category bip39
 */
export const validateMnemonic = mnemonic => BIP39.validateMnemonic(mnemonic, wordlist);
/**
 ...
 * @category bip39
 */
export const fromMnemonic = (mnemonic, password) => {
 ...
 * @category bip32
 */
export const derive = (privateKey, path) => {
 ...
 * @category cardano
 */
export const fromMnemonicCardano = (mnemonic, options) => {
  ...
  const indices = Bip32PrivateKey.CardanoPath.indices(options?.account ?? 0, options?.role ?? 0, options?.index ?? 0);
...
export const CardanoPath = {
   ...
   */
  create: (account = 0, role = 0, index = 0) => `m/1852'/${1815}'/${account}'/${role}/${index}`,
  /**
   * Payment key path (role = 0)
   */
  payment: (account = 0, index = 0) => CardanoPath.create(account, 0, index),
  /**
   * Stake key path (role = 2)
   */
  stake: (account = 0, index = 0) => CardanoPath.create(account, 2, index)
};
// ============================================================================
...
(function (Either) {
   ...
   * @category bip39
   */
  Either.fromMnemonic = (mnemonic, password) => E.gen(function* () {
    if (!validateMnemonic(mnemonic)) {
    ...
    return yield* E.mapLeft(ParseResult.decodeEither(FromBytes)(hdKey.privateKey), error => new PrivateKeyError(`Failed to decode private key: ${error}`));
  });
  /**
   ...
   * @category bip32
   */
  Either.derive = (privateKey, path) => E.gen(function* () {
    const privateKeyBytes = yield* E.mapLeft(ParseResult.encodeEither(FromBytes)(privateKey), error => new PrivateKeyError(`Failed to encode private key: ${error}`));
    ...
    return yield* E.mapLeft(ParseResult.decodeEither(FromBytes)(childKey.privateKey), error => new PrivateKeyError(`Failed to decode derived private key: ${error}`));
  });
  /**
   ...
   * @category cryptography
   */
  Either.sign = (privateKey, message) => E.gen(function* () {
    const privateKeyBytes = yield* E.mapLeft(ParseResult.encodeEither(FromBytes)(privateKey), error => new PrivateKeyError(`Failed to encode private key: ${error}`));
    if (privateKeyBytes.length === 64) {
      ...
      return new Ed25519Signature.Ed25519Signature({
        bytes: signature
      });
    }
