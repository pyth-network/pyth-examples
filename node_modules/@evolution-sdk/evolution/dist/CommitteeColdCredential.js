 ...
 */
export const FromBytes = /*#__PURE__*/Schema.transformOrFail(Schema.Uint8ArrayFromSelf, Schema.typeSchema(Credential.Credential), {
  strict: true,
  encode: (toI, _, ast) => Effect.gen(function* () {
    // Encode: Credential → 29 bytes with cc_cold header
    const credBytes = toI.hash;
    if (credBytes.length !== 28) {
      return yield* ParseResult.fail(new ParseResult.Type(ast, toI, `Invalid credential hash length: expected 28 bytes, got ${credBytes.length}`));
    }
  ...
  decode: (fromA, _, ast) => Effect.gen(function* () {
    // Decode: 29 bytes → Credential
    if (fromA.length !== 29) {
      return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid cc_cold credential length: expected 29 bytes, got ${fromA.length}`));
    }
    ...
    if (keyType !== 0x01) {
      return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid key type in header: expected 0x01 (CC Cold), got 0x0${keyType.toString(16)}`));
    }
    if (credType === 0x0c) {
    ...
    return yield* ParseResult.fail(new ParseResult.Type(ast, fromA, `Invalid credential type in header: expected 0x0C or 0x0D, got 0x0${credType.toString(16)}`));
  })
}).annotations({
 ...
 * @category transformations
 */
export const FromHex = /*#__PURE__*/Schema.compose(Schema.Uint8ArrayFromHex, FromBytes).annotations({
 ...
 * @category transformations
 */
export const FromBech32 = /*#__PURE__*/Schema.transformOrFail(Schema.String, Schema.typeSchema(Credential.Credential), {
  strict: true,
  encode: (_, __, ___, toA) => Effect.gen(function* () {
...
//# sourceMappingURL=CommitteeColdCredential.js.map