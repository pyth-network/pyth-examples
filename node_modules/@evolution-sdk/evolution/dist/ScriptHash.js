 ...
 * @category schemas
 */
export class ScriptHash extends /*#__PURE__*/Schema.TaggedClass()("ScriptHash", {
 ...
 * @category schemas
 */
export const FromBytes = /*#__PURE__*/Schema.transform(Schema.typeSchema(Hash28.BytesFromHex), Schema.typeSchema(ScriptHash), {
  strict: true,
  decode: bytes => new ScriptHash({
    hash: bytes
  }, {
    disableValidation: true
  }),
  // Disable validation since we already check length in Hash28
 ...
 * @category schemas
 */
export const FromHex = /*#__PURE__*/Schema.compose(Hash28.BytesFromHex, FromBytes).annotations({
 ...
 */
export const fromScript = script => {
  let tag;
  let body;
  switch (script._tag) {
    // Plutus script cases
    ...
    case "NativeScript":
      ...
      break;
    default:
      throw new Error(`Unknown script type: ${script._tag}`);
  }
  const prefixed = new Uint8Array(1 + body.length);
  prefixed[0] = tag;
  prefixed.set(body, 1);
  const hashBytes = blake2b(prefixed, {
    dkLen: 28
  });
