 ...
 * @category model
 */
export class HeaderBody extends /*#__PURE__*/Schema.TaggedClass()("HeaderBody", {
  blockNumber: Numeric.Uint64Schema,
  slot: Numeric.Uint64Schema,
  prevHash: /*#__PURE__*/Schema.NullOr(BlockHeaderHash.BlockHeaderHash),
  ...
  [Equal.symbol](that) {
    return that instanceof HeaderBody && this.blockNumber === that.blockNumber && this.slot === that.slot && this.prevHash === that.prevHash && Equal.equals(this.issuerVkey, that.issuerVkey) && Equal.equals(this.vrfVkey, that.vrfVkey) && Equal.equals(this.vrfResult, that.vrfResult) && this.blockBodySize === that.blockBodySize && Equal.equals(this.blockBodyHash, that.blockBodyHash) && Equal.equals(this.operationalCert, that.operationalCert) && Equal.equals(this.protocolVersion, that.protocolVersion);
  }
  [Hash.symbol]() {
    return Hash.cached(this, Hash.combine(Hash.hash("HeaderBody"))(Hash.combine(Hash.hash(this.blockNumber))(Hash.combine(Hash.hash(this.slot))(Hash.combine(Hash.hash(this.prevHash))(Hash.combine(Hash.hash(this.issuerVkey))(Hash.combine(Hash.hash(this.vrfVkey))(Hash.combine(Hash.hash(this.vrfResult))(Hash.combine(Hash.hash(this.blockBodySize))(Hash.combine(Hash.hash(this.blockBodyHash))(Hash.combine(Hash.hash(this.operationalCert))(Hash.hash(this.protocolVersion))))))))))));
  }
}
/**
...
export const arbitrary = /*#__PURE__*/FastCheck.record({
  ...
  vrfResult: FastCheck.record({
    output: FastCheck.uint8Array({
      minLength: 32,
      maxLength: 32
    }),
    proof: FastCheck.uint8Array({
      minLength: 80,
      maxLength: 80
    })
  }),
  ...
  vrfResult: new VrfCert.VrfCert({
    output: new VrfCert.VRFOutput({
      bytes: props.vrfResult.output
    }),
    proof: new VrfCert.VRFProof({
      bytes: props.vrfResult.proof
    })
  }),
 ...
 * @category schemas
 */
export const FromCDDL = /*#__PURE__*/Schema.transformOrFail(Schema.Tuple(CBOR.Integer,
// block_number as bigint
CBOR.Integer,
// slot as bigint
Schema.NullOr(CBOR.ByteArray),
// prev_hash as bytes or null
CBOR.ByteArray,
// issuer_vkey as bytes (32 bytes)
CBOR.ByteArray,
// vrf_vkey as bytes (32 bytes)
VrfCert.CDDLSchema,
// vrf_result as VrfCert
CBOR.Integer,
// block_body_size as bigint
CBOR.ByteArray,
// block_body_hash as bytes
OperationalCert.CDDLSchema,
// operational_cert as OperationalCert
ProtocolVersion.CDDLSchema // protocol_version as ProtocolVersion
  ...
  encode: toA => Eff.gen(function* () {
    ...
    return [BigInt(toA.blockNumber), BigInt(toA.slot), prevHashBytes, issuerVkeyBytes, vrfVkeyBytes, [vrfOutputBytes, vrfProofBytes], BigInt(toA.blockBodySize), blockBodyHashBytes, [hotVkeyBytes, BigInt(toA.operationalCert.sequenceNumber), BigInt(toA.operationalCert.kesPeriod), sigmaBytes], [BigInt(toA.protocolVersion.major), BigInt(toA.protocolVersion.minor)]];
  }),
  decode: ([rawBlockNumber, rawSlotNumber, prevHashBytes, issuerVkeyBytes, vrfVkeyBytes, [vrfOutputBytes, vrfProofBytes], rawBlockBodySize, blockBodyHashBytes, [hotVkeyBytes, sequenceNumber, kesPeriod, sigmaBytes], [protocolMajor, protocolMinor]]) => Eff.gen(function* () {
    ...
    const operationalCert = yield* ParseResult.decode(OperationalCert.FromCDDL)([hotVkeyBytes, sequenceNumber, kesPeriod, sigmaBytes]);
 ...
 * @category predicates
 */
export const isHeaderBody = /*#__PURE__*/Schema.is(HeaderBody);
/**
 ...
 * @category schemas
 */
export const FromCBORBytes = (options = CBOR.CML_DEFAULT_OPTIONS) => Schema.compose(CBOR.FromBytes(options),
// Uint8Array → CBOR
 ...
 * @category schemas
 */
export const FromCBORHex = (options = CBOR.CML_DEFAULT_OPTIONS) => Schema.compose(Schema.Uint8ArrayFromHex,
// string → Uint8Array
 ...
 * @category conversion
 */
export const fromCBORBytes = (bytes, options) => Schema.decodeSync(FromCBORBytes(options))(bytes);
/**
 ...
 * @category conversion
 */
export const fromCBORHex = (hex, options) => Schema.decodeSync(FromCBORHex(options))(hex);
/**
 ...
 * @category conversion
 */
export const toCBORBytes = (headerBody, options) => Schema.encodeSync(FromCBORBytes(options))(headerBody);
/**
 ...
 * @category conversion
 */
export const toCBORHex = (headerBody, options) => Schema.encodeSync(FromCBORHex(options))(headerBody);
