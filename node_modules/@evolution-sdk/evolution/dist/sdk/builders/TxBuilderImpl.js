 ...
 * @category helpers
 */
export const isScriptAddressCore = address => {
 ...
 * @category helpers
 */
export const filterScriptUtxos = utxos => Effect.gen(function* () {
  const scriptUtxos = [];
 ...
 * @category helpers
 */
export const calculateTotalAssets = utxos => {
  const utxoArray = globalThis.Array.isArray(utxos) ? utxos : globalThis.Array.from(utxos);
  return utxoArray.reduce((total, utxo) => CoreAssets.merge(total, utxo.assets), CoreAssets.zero);
};
/**
 ...
 * @category helpers
 */
export const tierRefScriptFee = (multiplier, sizeIncrement, baseFee, totalSize) => {
  let acc = 0;
 ...
 * @category helpers
 */
export const calculateReferenceScriptFee = (utxos, costPerByte) => Effect.gen(function* () {
...
// ============================================================================
.pipe(Effect.mapError(error => new TransactionBuilderError({
 ...
 * @category helpers
 */
export const makeTxOutput = params => Effect.gen(function* () {
  // Convert Script to ScriptRef for CBOR encoding if provided
  const scriptRefEncoded = params.scriptRef ? new ScriptRef.ScriptRef({
    bytes: CoreScript.toCBOR(params.scriptRef)
  }) : undefined;
 ...
 * @internal
 */
export const txOutputToTransactionOutput = params => Effect.gen(function* () {
  // Convert Script to ScriptRef for CBOR encoding if provided
  const scriptRefEncoded = params.scriptRef ? new ScriptRef.ScriptRef({
    bytes: CoreScript.toCBOR(params.scriptRef)
  }) : undefined;
 ...
 * @category helpers
 */
export const mergeAssetsIntoUTxO = (utxo, additionalAssets) => Effect.gen(function* () {
 ...
 * @category helpers
 */
export const mergeAssetsIntoOutput = (output, additionalAssets) => Effect.gen(function* () {
  // Merge assets using Core Assets helper
 ...
 * @category assembly
 */
export const buildTransactionInputs = utxos => Effect.gen(function* () {
 ...
 * @category assembly
 */
export const assembleTransaction = (inputs, outputs, fee) => Effect.gen(function* () {
  ...
  let totalCollateral;
  if (state.collateral) {
    yield* Effect.logDebug(`[Assembly] Adding collateral: ${state.collateral.inputs.length} inputs, ` + `total ${state.collateral.totalAmount} lovelace`);
    // Collateral phase guarantees at least one input for script transactions
    collateralInputs = yield* buildTransactionInputs(state.collateral.inputs);
    ...
    if (state.collateral.returnOutput) {
      yield* Effect.logDebug(`[Assembly] Collateral return lovelace: ${state.collateral.returnOutput.assets.lovelace}`);
  ...
  const nativeScripts = []; // TODO: Add native script type
  ...
  const mintIndexMap = new Map();
  if (state.mint && state.mint.map.size > 0) {
    // Get sorted policy IDs (Cardano redeemer indices require sorted order)
    const sortedPolicyIds = globalThis.Array.from(state.mint.map.keys()).map(pid => PolicyId.toHex(pid)).sort();
  ...
  for (const [key, redeemerData] of state.redeemers) {
    ...
    if (redeemerData.tag === "mint") {
      ...
      for (let i = 0; i < state.certificates.length; i++) {
        ...
        if ("stakeCredential" in cert && cert.stakeCredential) {
          const certCredHex = Bytes.toHex(cert.stakeCredential.hash);
    ...
    yield* Effect.logDebug(`[Assembly] Redeemer exUnits before creating: mem=${redeemerData.exUnits?.mem ?? 0n}, steps=${redeemerData.exUnits?.steps ?? 0n}`);
    ...
    const redeemer = new Redeemer.Redeemer({
      ...
      // Use actual redeemer index
      data: redeemerData.data,
      exUnits: redeemerData.exUnits ? new Redeemer.ExUnits({
        mem: redeemerData.exUnits.mem,
        steps: redeemerData.exUnits.steps
      }) : new Redeemer.ExUnits({
        mem: 0n,
        steps: 0n
      }) // will be updated by script evaluation
    });
    yield* Effect.logDebug(`[Assembly] Created redeemer: tag=${redeemer.tag}, index=${redeemer.index}, exUnits=[${redeemer.exUnits.mem}, ${redeemer.exUnits.steps}]`);
  ...
  if (redeemers.length > 0) {
    ...
    const fullProtocolParams = yield* config.provider.Effect.getProtocolParameters().pipe(Effect.mapError(providerError => new TransactionBuilderError({
    ...
    const plutusV1Costs = hasPlutusV1 ? Object.values(fullProtocolParams.costModels.PlutusV1).map(v => BigInt(v)) : []; // Empty array = not included in language_views
    const plutusV2Costs = hasPlutusV2 ? Object.values(fullProtocolParams.costModels.PlutusV2).map(v => BigInt(v)) : []; // Empty array = not included in language_views
    const plutusV3Costs = hasPlutusV3 ? Object.values(fullProtocolParams.costModels.PlutusV3).map(v => BigInt(v)) : []; // Empty array = not included in language_views
    ...
    scriptDataHash = hashScriptData(redeemersConcrete, costModels, plutusDataArray.length > 0 ? plutusDataArray : undefined);
  ...
  const certificates = state.certificates.length > 0 ? state.certificates : undefined;
  // Build withdrawals (Withdrawals object or undefined)
  const withdrawals = state.withdrawals.size > 0 ? new Withdrawals.Withdrawals({
  ...
  const requiredSigners = state.requiredSigners.length > 0 ? state.requiredSigners : undefined;
  if (requiredSigners) {
  ...
  const body = new TransactionBody.TransactionBody({
    ...
    fee,
    // Now using actual calculated fee, not placeholder
    ttl,
    // Transaction expiration slot
    validityIntervalStart,
    // Transaction valid-from slot
    collateralInputs,
    // Collateral inputs from Collateral phase
    collateralReturn,
    // Collateral return output from Collateral phase
    totalCollateral,
    // Total collateral amount from Collateral phase
    referenceInputs,
    // Reference inputs for reading on-chain data (undefined if none)
    mint: state.mint && state.mint.map.size > 0 ? state.mint : undefined,
    // Mint field from minting operations
    scriptDataHash,
    // Hash of redeemers + datums + cost models (required for Plutus scripts)
    auxiliaryDataHash,
    // Hash of auxiliary data (required when metadata is present)
    certificates,
    // Certificates for staking operations
    withdrawals,
    // Withdrawals for claiming staking rewards
    requiredSigners,
    // Extra signers required for script validation
    votingProcedures: state.votingProcedures,
    // Voting procedures for governance voting
  ...
  const transaction = new Transaction.Transaction({
    ...
    witnessSet,
    isValid: true,
    // Assume valid until script evaluation proves otherwise
 ...
 */
export const calculateTransactionSize = transaction => Effect.gen(function* () {
 ...
 * @category fee-calculation
 */
export const calculateMinimumFee = (transactionSizeBytes, protocolParams) => {
  const {
    minFeeCoefficient,
    minFeeConstant
  } = protocolParams;
 ...
 * @internal
 */
export const extractPaymentKeyHash = address => Effect.gen(function* () {
  const addressStructure = yield* Effect.try({
    try: () => CoreAddress.fromBech32(address),
    catch: error => new TransactionBuilderError({
      message: `Failed to parse address ${address}`,
 ...
 * @internal
 */
const extractPaymentKeyHashFromCore = address => {
  // Check if payment credential is a KeyHash
 ...
 * @internal
 */
const buildFakeVKeyWitness = keyHash => Effect.gen(function* () {
  ...
  const signature = yield* Effect.try({
    try: () => new Ed25519Signature.Ed25519Signature({
 ...
 * @category fee-calculation
 */
export const buildFakeWitnessSet = inputUtxos => Effect.gen(function* () {
  const stateRef = yield* TxContext;
  const state = yield* Ref.get(stateRef);
  // Extract unique key hashes from input addresses (Core Address)
  const keyHashesSet = new Set();
  const keyHashes = [];
  ...
  const addNativeScriptWitnesses = script => {
  ...
  for (const [_key, redeemerData] of state.redeemers) {
    ...
    const exUnits = redeemerData.exUnits ?? {
      mem: 0n,
      steps: 0n
    };
    ...
    fakeRedeemers.push(new Redeemer.Redeemer({
      tag: redeemerData.tag,
      index: fakeIndex++,
      // Unique placeholder, will be set correctly in assembly
      data: redeemerData.data,
      exUnits: new Redeemer.ExUnits({
        mem: exUnits.mem,
        steps: exUnits.steps
      })
    }));
 ...
 * @category fee-calculation
 */
export const calculateFeeIteratively = (inputUtxos, inputs, outputs, redeemers, protocolParams) => Effect.gen(function* () {
  ...
  if (state.collateral) {
    ...
    if (builtCollateralInputs.length > 0) {
      collateralInputs = builtCollateralInputs;
    }
    collateralReturn = state.collateral.returnOutput;
    totalCollateral = state.collateral.totalAmount;
  }
  ...
  const hasPlutusScripts = fakeWitnessSet.plutusV1Scripts && fakeWitnessSet.plutusV1Scripts.length > 0 || fakeWitnessSet.plutusV2Scripts && fakeWitnessSet.plutusV2Scripts.length > 0 || fakeWitnessSet.plutusV3Scripts && fakeWitnessSet.plutusV3Scripts.length > 0 || state.redeemers.size > 0;
  ...
  const placeholderScriptDataHash = hasPlutusScripts ? new ScriptDataHash.ScriptDataHash({
  ...
  if (state.referenceInputs.length > 0) {
    const refInputs = yield* buildTransactionInputs(state.referenceInputs);
    referenceInputsForFee = refInputs;
  }
  ...
  while (iterations < maxIterations) {
    ...
    const body = new TransactionBody.TransactionBody({
      ...
      fee: currentFee,
      ttl,
      // Include TTL for accurate size calculation
      validityIntervalStart,
      // Include validity start for accurate size calculation
      mint,
      // Include mint field for accurate size calculation
      scriptDataHash: placeholderScriptDataHash,
      // Include scriptDataHash for accurate size
      auxiliaryDataHash: placeholderAuxiliaryDataHash,
      // Include auxiliaryDataHash for accurate size
      collateralInputs,
      // Include collateral for accurate size
      collateralReturn,
      // Include collateral return for accurate size
      totalCollateral,
      // Include total collateral for accurate size
      certificates,
      // Include certificates for accurate size calculation
      withdrawals,
      // Include withdrawals for accurate size calculation
      requiredSigners,
      // Include requiredSigners for accurate size calculation
      referenceInputs: referenceInputsForFee,
      // Include reference inputs for accurate size calculation
      votingProcedures: state.votingProcedures,
      // Include voting procedures for accurate size calculation
    ...
    const transaction = new Transaction.Transaction({
      ...
      // Use fake witness set for accurate size
      isValid: true,
      auxiliaryData: state.auxiliaryData ?? null
    });
    // Calculate size
    ...
    if (currentFee === previousFee && size === previousSize && currentFee >= calculatedFee) {
      if (iterations > 1) {
        yield* Effect.logDebug(`Fee converged after ${iterations} iterations: ${currentFee} lovelace (tx size: ${size} bytes)`);
      }
      return currentFee;
    }
 ...
 * @category fee-calculation
 */
export const verifyTransactionBalance = (selectedUtxos, outputs, fee) => {
  ...
  for (const unit of CoreAssets.getUnits(balance)) {
    if (unit !== "lovelace") {
      ...
      if (amount < 0n) {
        ...
        lovelaceShortfall = lovelaceShortfall > 0n ? lovelaceShortfall : 100000n;
        break;
      }
    }
  }
 ...
 * @category validation
 */
export const validateTransactionBalance = params => Effect.gen(function* () {
  const {
    fee,
    totalInputAssets,
    totalOutputAssets
 ...
 * @category change
 */
export const calculateMinimumUtxoLovelace = params => Effect.gen(function* () {
  const calculateRequiredLovelace = lovelace => Effect.gen(function* () {
 ...
 * @category change
 */
export const createChangeOutput = params => Effect.gen(function* () {
  const {
    changeAddress,
    coinsPerUtxoByte,
    leftoverAssets,
  ...
  if (unfrackOptions) {
    const unfrackedOutputs = yield* Unfrack.createUnfrackedChangeOutputs(changeAddress, leftoverAssets, unfrackOptions, coinsPerUtxoByte).pipe(Effect.mapError(error => new TransactionBuilderError({
  ...
  yield* Effect.logDebug(`[createChangeOutput] Leftover: ${leftoverLovelace} lovelace, MinUTxO: ${minLovelace} lovelace`);
...
//# sourceMappingURL=TxBuilderImpl.js.map