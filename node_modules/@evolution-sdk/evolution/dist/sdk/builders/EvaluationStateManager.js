 ...
 */
export const invalidateExUnits = redeemers => {
 ...
 * @category state-management
 */
export const hasUnevaluatedRedeemers = redeemers => {
  return Array.from(redeemers.values()).some(redeemer => !redeemer.exUnits || redeemer.exUnits.mem === 0n || redeemer.exUnits.steps === 0n);
};
/**
 ...
 * @category state-management
 */
export const allRedeemersEvaluated = redeemers => {
  if (redeemers.size === 0) return false;
  return Array.from(redeemers.values()).every(redeemer => redeemer.exUnits && redeemer.exUnits.mem > 0n && redeemer.exUnits.steps > 0n);
};
//# sourceMappingURL=EvaluationStateManager.js.map