...
export const makeSignBuilder = params => {
  ...
  const chainResult = () => {
    ...
    const created = outputs.map((output, index) => new CoreUTxO.UTxO({
  ...
  const signEffect = {
    ...
    sign: () => Effect.gen(function* () {
      ...
      const walletWitnessSet = yield* wallet.Effect.signTx(transaction, {
        utxos,
        referenceUtxos
      }).pipe(Effect.mapError(walletError => new TransactionBuilderError({
     ...
     */
    signAndSubmit: () => Effect.gen(function* () {
     ...
     */
    signWithWitness: witnessSet => Effect.gen(function* () {
     ...
     */
    assemble: witnesses => Effect.gen(function* () {
      ...
      const mergedWitnessSet = witnesses.reduce((acc, ws) => new TransactionWitnessSet.TransactionWitnessSet({
     ...
     */
    partialSign: () => Effect.gen(function* () {
  ...
  return {
    ...
    signWithWitness: witnessSet => Effect.runPromise(signEffect.signWithWitness(witnessSet)),
    assemble: witnesses => Effect.runPromise(signEffect.assemble(witnesses)),
...
//# sourceMappingURL=SignBuilderImpl.js.map