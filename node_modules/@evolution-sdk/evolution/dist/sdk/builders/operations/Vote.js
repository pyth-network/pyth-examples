...
export const createVoteProgram = params => Effect.gen(function* () {
  ...
  yield* Ref.update(ctx, state => {
    ...
    const mergedMap = new Map(mergedVotingProcedures.procedures);
    for (const [voter, govActionMap] of params.votingProcedures.procedures.entries()) {
      ...
      if (existingGovActionMap) {
        ...
        const mergedGovActionMap = new Map(existingGovActionMap);
  ...
  yield* Effect.logDebug(`[Vote] Added voting procedures for ${params.votingProcedures.procedures.size} voter(s), ${scriptVoters.size} script-controlled`);
});
