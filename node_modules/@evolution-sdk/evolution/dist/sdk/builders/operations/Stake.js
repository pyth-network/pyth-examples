 ...
 * @category programs
 */
export const createRegisterStakeProgram = params => Effect.gen(function* () {
  ...
  const protocolParams = yield* config.provider.Effect.getProtocolParameters().pipe(Effect.mapError(err => new TransactionBuilderError({
 ...
 * @category programs
 */
export const createDelegateToProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  ...
  const delegationType = params.poolKeyHash && params.drep ? "StakeVoteDelegCert (pool + DRep)" : params.poolKeyHash ? "StakeDelegation (pool)" : "VoteDelegCert (DRep)";
 ...
 * @category programs
 */
export const createDelegateToPoolProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.stakeCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
 ...
 * @category programs
 */
export const createDelegateToDRepProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.stakeCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
 ...
 * @category programs
 */
export const createDelegateToPoolAndDRepProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.stakeCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
 ...
 * @category programs
 */
export const createRegisterAndDelegateToProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  const config = yield* TxBuilderConfigTag;
  // Validate at least one delegation target
  if (!params.poolKeyHash && !params.drep) {
  ...
  const delegationType = params.poolKeyHash && params.drep ? "StakeVoteRegDelegCert (pool + DRep)" : params.poolKeyHash ? "StakeRegDelegCert (pool)" : "VoteRegDelegCert (DRep)";
 ...
 * @category programs
 */
export const createDeregisterStakeProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  const config = yield* TxBuilderConfigTag;
  // Check if script-controlled
  const isScriptControlled = params.stakeCredential._tag === "ScriptHash";
 ...
 * @category programs
 */
export const createWithdrawProgram = (params, config) => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.stakeCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
  ...
  yield* Effect.logDebug(`[Withdraw] Added withdrawal of ${params.amount} lovelace from ${Bytes.toHex(params.stakeCredential.hash)}`);
});
