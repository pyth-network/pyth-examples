 ...
 * @category programs
 */
export const createRegisterDRepProgram = params => Effect.gen(function* () {
  ...
  yield* Effect.logDebug(`[RegisterDRep] Added RegDrepCert for DRep ${Bytes.toHex(params.drepCredential.hash)} with deposit ${drepDeposit}`);
});
/**
 ...
 * @category programs
 */
export const createUpdateDRepProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.drepCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
 ...
 * @category programs
 */
export const createDeregisterDRepProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  const config = yield* TxBuilderConfigTag;
  // Get drepDeposit from protocol parameters via provider
  if (!config.provider) {
  ...
  yield* Effect.logDebug(`[DeregisterDRep] Added UnregDrepCert for DRep ${Bytes.toHex(params.drepCredential.hash)} with refund ${drepDeposit}`);
});
// ============================================================================
 ...
 * @category programs
 */
export const createAuthCommitteeHotProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  ...
  yield* Effect.logDebug(`[AuthCommitteeHot] Added AuthCommitteeHotCert for cold ${Bytes.toHex(params.coldCredential.hash)} to hot ${Bytes.toHex(params.hotCredential.hash)}`);
});
/**
 ...
 * @category programs
 */
export const createResignCommitteeColdProgram = params => Effect.gen(function* () {
  const ctx = yield* TxContext;
  // Check if script-controlled
  const isScriptControlled = params.coldCredential._tag === "ScriptHash";
  if (isScriptControlled && !params.redeemer) {
  ...
  yield* Effect.logDebug(`[ResignCommitteeCold] Added ResignCommitteeColdCert for cold credential ${Bytes.toHex(params.coldCredential.hash)}`);
});
