 ...
 */
export const extractTokens = assets => {
  const tokens = [];
 ...
 */
export const groupByPolicy = tokens => {
 ...
 */
const calculateBundleMinUTxO = (bundleTokens, changeAddress, coinsPerUtxoByte) => Effect.gen(function* () {
 ...
 */
export const calculateTokenBundles = (tokens, options, changeAddress, coinsPerUtxoByte) => Effect.gen(function* () {
 ...
 */
const bundleTokensWithRules = (tokens, bundleSize, changeAddress, coinsPerUtxoByte) => Effect.gen(function* () {
  const bundles = [];
 ...
 */
export const calculateAdaSubdivision = (leftoverAda, options) => {
  return Effect.gen(function* () {
    const threshold = options.ada?.subdivideThreshold ?? 100000000n;
 ...
 * @category builders
 */
export const createUnfrackedChangeOutputs = (changeAddress, changeAssets, options = {}, coinsPerUtxoByte) => {
  return Effect.gen(function* () {
    ...
    if (tokens.length === 0) {
      ...
      if (availableLovelace >= subdivideThreshold) {
        ...
        const adaMinUTxO = yield* calculateMinimumUtxoLovelace({
          address: changeAddress,
          assets: CoreAssets.fromLovelace(1000000n),
          // Use 1 ADA for minUTxO estimation
          coinsPerUtxoByte
        });
    ...
    for (const [_policyId, policyTokens] of tokensByPolicy) {
      ...
      if (policyTokens.length <= bundleSize) {
        ...
        const minUTxO = yield* calculateMinimumUtxoLovelace({
          address: changeAddress,
          assets: CoreAssets.merge(bundleAssets, CoreAssets.fromLovelace(1000000n)),
          // Add 1 ADA for calculation
          coinsPerUtxoByte
        });
        bundles.push({
          tokens: policyTokens,
    ...
    if (remaining < 0n) {
      yield* Effect.logDebug(`[Unfrack] Insufficient lovelace for ${bundles.length} bundles! ` + `Need ${bundlesMinUTxO}, have ${availableLovelace}. ` + `Falling back to single change output (guaranteed affordable by ChangeCreation pre-flight check).`);
...
//# sourceMappingURL=Unfrack.js.map