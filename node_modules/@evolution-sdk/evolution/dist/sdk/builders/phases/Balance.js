 ...
 */
export const executeBalance = () => Effect.gen(function* () {
  ...
  const changeAssets = buildCtx.changeOutputs.reduce((acc, output) => CoreAssets.merge(acc, output.assets), CoreAssets.zero);
  ...
  yield* Effect.logDebug(`[Balance] Inputs: ${formatAssetsForLog(inputAssets)}, ` + `Outputs: ${formatAssetsForLog(outputAssets)}, ` + `Change: ${formatAssetsForLog(changeAssets)}, ` + `Fee: ${buildCtx.calculatedFee}, ` + `Delta: ${formatAssetsForLog(delta)}, ` + `Balanced: ${isBalanced}`);
  ...
  if (deltaLovelace > 0n) {
    ...
    if (isDrainToMode) {
      ...
      const updatedOutput = new targetOutput.constructor({
      ...
      const newTotalOutputAssets = newOutputs.reduce((acc, output) => CoreAssets.merge(acc, output.assets), CoreAssets.zero);
      yield* Ref.update(ctx, s => ({
        ...s,
        outputs: newOutputs,
        totalOutputAssets: newTotalOutputAssets
      }));
      yield* Effect.logDebug(`[Balance] DrainTo mode: Merged ${deltaLovelace} lovelace into output[${drainToIndex}]. ` + `New output value: ${CoreAssets.lovelaceOf(newAssets)}. Transaction balanced.`);
      return {
        next: "complete"
      };
    } else if (isBurnMode) {
      // Burn mode: Positive delta is the burned leftover (becomes implicit fee)
      yield* Effect.logDebug(`[Balance] Burn mode: ${deltaLovelace} lovelace burned as implicit fee. ` + `Transaction balanced.`);
      return {
        next: "complete"
      };
    } else {
      ...
      yield* Effect.logDebug(`[Balance] Positive delta detected: ${deltaLovelace} lovelace. ` + `Likely from fee reduction after output count change. ` + `Routing back to changeCreation for convergence.`);
      return {
        next: "changeCreation"
      };
    }
  ...
  yield* Effect.logDebug(`[Balance] Shortfall detected: ${-deltaLovelace} lovelace. ` + `Returning to changeCreation to adjust change output.`);
  return {
    next: "changeCreation"
  };
});
