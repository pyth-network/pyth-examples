 ...
 */
const getAvailableUtxos = (allUtxos, selectedUtxos) => {
 ...
 */
const createChangeOutputs = (leftoverAfterFee, changeAddress, coinsPerUtxoByte) => Effect.gen(function* () {
  ...
  const changeOutputs = yield* Unfrack.createUnfrackedChangeOutputs(changeAddress, leftoverAfterFee, unfrackOptions, coinsPerUtxoByte).pipe(Effect.mapError(err => new TransactionBuilderError({
 ...
 */
export const executeChangeCreation = () => Effect.gen(function* () {
  ...
  const {
    deposits: certificateDeposits,
    refunds: certificateRefunds
  } = calculateCertificateBalance(state.certificates, state.poolDeposits);
  ...
  if (state.sendAllTo !== undefined) {
    yield* Effect.logDebug(`[ChangeCreation] SendAll mode: Creating output to ${state.sendAllTo} with ` + `${formatAssetsForLog(tentativeLeftover)}`);
    ...
    if (leftoverLovelace < sendAllMinUtxo) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: `sendAll() failed: Insufficient funds to create valid output. ` + `Available: ${leftoverLovelace} lovelace, Required: ${sendAllMinUtxo} lovelace (minUTxO). ` + `The wallet balance after fees is too low to create a valid UTxO.`
      }));
    }
  ...
  if (leftoverLovelace < 0n) {
    ...
    yield* Effect.logDebug(`[ChangeCreation] Insufficient lovelace for fee: ${leftoverLovelace}. ` + `Shortfall: ${shortfall}. Returning to selection.`);
    yield* Ref.update(buildCtxRef, ctx => ({
      ...ctx,
      shortfall,
      changeOutputs: []
  ...
  if (leftoverLovelace < minLovelaceForSingle) {
    ...
    if (hasMoreUtxos && buildCtx.attempt < MAX_ATTEMPTS) {
      yield* Effect.logDebug(`[ChangeCreation] Leftover ${leftoverLovelace} < ${minLovelaceForSingle} minUTxO ` + `(shortfall: ${shortfall}${isAdaOnlyLeftover ? ", ADA-only" : ", with native assets"}). ` + `Attempting reselection (${buildCtx.attempt + 1}/${MAX_ATTEMPTS})`);
      yield* Ref.update(buildCtxRef, ctx => ({
        ...ctx,
        shortfall,
        changeOutputs: []
    ...
    yield* Effect.logDebug(`[ChangeCreation] Cannot reselect: ${!hasMoreUtxos ? "No more UTxOs available" : `MAX_ATTEMPTS (${MAX_ATTEMPTS}) exhausted`}. ` + `Leftover (before fee): ${formatAssetsForLog(tentativeLeftover)}, minUTxO: ${minLovelaceForSingle}`);
    // CASE 1: Native assets present - cannot use drain/burn fallback
    if (!isAdaOnlyLeftover) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: `Cannot balance transaction: Native assets present in leftover ` + `but insufficient lovelace (${leftoverLovelace} < ${minLovelaceForSingle} minUTxO) ` + `after ${buildCtx.attempt} selection attempts.\n\n` + `Your leftover includes native assets (tokens) which require at least ` + `${minLovelaceForSingle} lovelace to create a valid change output, but only ` + `${leftoverLovelace} lovelace remains.\n\n` + `Solutions:\n` + `1. Include the native assets in your payment outputs\n` + `2. Add more lovelace to your wallet\n` + `3. Use fewer/smaller outputs to reduce fees`,
        cause: undefined
      }));
    }
  ...
  if (buildOptions.unfrack && buildCtx.canUnfrack) {
    const protocolParams = yield* ProtocolParametersTag;
    const changeOutputs = yield* createChangeOutputs(tentativeLeftover, changeAddress, protocolParams.coinsPerUtxoByte);
...
//# sourceMappingURL=ChangeCreation.js.map