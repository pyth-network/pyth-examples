...
import { BuildOptionsTag, EvaluationError, PhaseContextTag, TransactionBuilderError, TxBuilderConfigTag, TxContext } from "../TransactionBuilder.js";
 ...
 */
const buildCostModels = protocolParams => Effect.gen(function* () {
 ...
 */
const enrichFailuresWithLabels = (failures, redeemers, inputIndexMapping, withdrawalIndexMapping, mintIndexMapping, certIndexMapping, voteIndexMapping) => {
 ...
 *
 */
const resolveDeferredRedeemers = (deferredRedeemers, sortedUtxos, inputIndexMapping) => Effect.gen(function* () {
 ...
 */
export const executeEvaluation = () => Effect.gen(function* () {
  ...
  if (!evaluator) {
    return yield* Effect.fail(new TransactionBuilderError({
      ...
      cause: {
        redeemerCount: state.redeemers.size
      }
    }));
  }
  ...
  yield* Effect.logDebug(`[Evaluation] Processing ${state.redeemers.size} resolved + ${state.deferredRedeemers.size} deferred redeemer(s)`);
  ...
  const inputIndexMapping = new Map(); // index -> "txHash#outputIndex"
  ...
  const mintIndexMapping = new Map();
  if (updatedState.mint && updatedState.mint.map.size > 0) {
    const sortedPolicyIds = Array.from(updatedState.mint.map.keys()).map(pid => PolicyId.toHex(pid)).sort();
  ...
  const certIndexMapping = new Map();
  ...
  const withdrawalIndexMapping = new Map();
  ...
  const voteIndexMapping = new Map();
  ...
  const evalResults = yield* evaluator.evaluate(transaction, additionalUtxos, evaluationContext).pipe(Effect.mapError(evalError => {
    // Enrich failures with labels from builder state
    // evalError.failures contains raw failures from evaluator (provider or aiken)
    const enrichedFailures = enrichFailuresWithLabels(evalError.failures ?? [], updatedState.redeemers, inputIndexMapping, withdrawalIndexMapping, mintIndexMapping, certIndexMapping, voteIndexMapping);
  ...
  if (updatedState.redeemers.size > 0 && evalResults.length === 0) {
    yield* Effect.logError(`[Evaluation] Expected evaluation results for ${updatedState.redeemers.size} redeemer(s) but received 0 results. ` + `This may indicate a provider schema parsing issue or network error.`);
    return yield* Effect.fail(new TransactionBuilderError({
  ...
  for (const evalRedeemer of evalResults) {
    if (evalRedeemer.redeemer_tag === "spend") {
      ...
      if (redeemer) {
        ...
        yield* Effect.logDebug(`[Evaluation] Updated redeemer at ${utxoRef} (spend:${evalRedeemer.redeemer_index}): ` + `mem=${evalRedeemer.ex_units.mem}, steps=${evalRedeemer.ex_units.steps}`);
      } else {
        return yield* Effect.fail(new TransactionBuilderError({
      ...
      if (redeemer) {
        ...
        yield* Effect.logDebug(`[Evaluation] Updated redeemer for policy ${policyIdHex} (mint:${evalRedeemer.redeemer_index}): ` + `mem=${evalRedeemer.ex_units.mem}, steps=${evalRedeemer.ex_units.steps}`);
      } else {
        return yield* Effect.fail(new TransactionBuilderError({
      ...
      if (redeemer) {
        ...
        yield* Effect.logDebug(`[Evaluation] Updated redeemer for cert ${certKey} (publish:${evalRedeemer.redeemer_index}): ` + `mem=${evalRedeemer.ex_units.mem}, steps=${evalRedeemer.ex_units.steps}`);
      } else {
        return yield* Effect.fail(new TransactionBuilderError({
      ...
      if (redeemer) {
        ...
        yield* Effect.logDebug(`[Evaluation] Updated redeemer for withdrawal ${rewardKey} (reward:${evalRedeemer.redeemer_index}): ` + `mem=${evalRedeemer.ex_units.mem}, steps=${evalRedeemer.ex_units.steps}`);
      } else {
        return yield* Effect.fail(new TransactionBuilderError({
      ...
      if (redeemer) {
        ...
        yield* Effect.logDebug(`[Evaluation] Updated redeemer for vote ${voterKey} (vote:${evalRedeemer.redeemer_index}): ` + `mem=${evalRedeemer.ex_units.mem}, steps=${evalRedeemer.ex_units.steps}`);
      } else {
        return yield* Effect.fail(new TransactionBuilderError({
