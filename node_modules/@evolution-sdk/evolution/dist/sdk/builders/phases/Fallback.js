 ...
 */
export const executeFallback = () => Effect.gen(function* () {
  ...
  if (buildOptions.drainTo !== undefined) {
    ...
    if (drainToIndex < 0 || drainToIndex >= outputs.length) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: `Invalid drainTo index: ${drainToIndex}. ` + `Transaction has ${outputs.length} output(s), valid indices are 0-${outputs.length - 1}.`,
        cause: {
          drainToIndex,
    ...
    yield* Effect.logDebug(`[Fallback] DrainTo strategy: Change outputs cleared. ` + `Leftover will be merged into output[${drainToIndex}] after fee calculation.`);
  ...
  if (buildOptions.onInsufficientChange === "burn") {
    ...
    yield* Effect.logDebug(`[Fallback] Burn strategy: Leftover will be burned as implicit fee ` + `(recalculating fee without change outputs).`);
  ...
  return yield* Effect.fail(new TransactionBuilderError({
    message: `[Fallback] CRITICAL BUG: Fallback phase reached without drainTo or burn configured. ` + `ChangeCreation should have prevented this.`,
    cause: {
