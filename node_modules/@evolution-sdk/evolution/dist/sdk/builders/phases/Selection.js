 ...
 */
const formatAssetsForLog = assets => {
  const parts = [`lovelace: ${CoreAssets.lovelaceOf(assets)}`];
 ...
 */
const getAvailableUtxos = (allUtxos, selectedUtxos, referenceInputs = []) => {
 ...
 */
const getCoinSelectionAlgorithm = algorithm => {
  switch (algorithm) {
    case "largest-first":
 ...
 */
const resolveCoinSelectionFn = coinSelection => {
 ...
 */
const addUtxosToState = selectedUtxos => Effect.gen(function* () {
  ...
  yield* Ref.update(ctx, state => {
    ...
    const updatedRedeemers = hasRedeemers ? EvaluationStateManager.invalidateExUnits(state.redeemers) : state.redeemers;
    return {
      ...state,
 ...
 */
const performCoinSelectionUpdateState = assetShortfalls => Effect.gen(function* () {
  const ctx = yield* TxContext;
  const state = yield* Ref.get(ctx);
 ...
 */
export const executeSelection = () => Effect.gen(function* () {
  const ctx = yield* TxContext;
  ...
  if (state.sendAllTo !== undefined) {
    ...
    if (state.outputs.length > 0) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with payToAddress(). " + "sendAll automatically creates the output with all wallet assets."
      }));
    }
    if (state.selectedUtxos.length > 0) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with collectFrom(). " + "sendAll automatically collects all wallet UTxOs."
      }));
    }
    if (state.mint !== undefined) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with mintAssets(). " + "sendAll is designed for draining a wallet, not minting operations."
      }));
    }
    if (state.certificates.length > 0) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with staking operations (registerStake, deregisterStake, delegateTo, etc.). " + "sendAll is designed for simple wallet drain operations only."
      }));
    }
    if (state.withdrawals.size > 0) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with withdraw(). " + "sendAll is designed for simple wallet drain operations only."
      }));
    }
    if (state.votingProcedures !== undefined || state.proposalProcedures !== undefined) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "sendAll() cannot be used with governance operations (vote, propose). " + "sendAll is designed for simple wallet drain operations only."
      }));
    }
    ...
    yield* Effect.logDebug(`[Selection] SendAll: Collected ${allAvailableUtxos.length} UTxO(s) with ` + `${formatAssetsForLog(totalInputAssets)}`);
  ...
  yield* Effect.logDebug(`[Selection] Needed: {${formatAssetsForLog(totalNeeded)}}, ` + `Available: {${formatAssetsForLog(inputAssets)}}, ` + `Delta: {${formatAssetsForLog(assetDelta)}}` + (isReselection ? `, Reselection: shortfall=${buildCtx.shortfall}` : ""));
  // Step 5: Perform selection or skip
  if (!needsSelection) {
    yield* Effect.logDebug("[Selection] Assets sufficient");
    const state = yield* Ref.get(ctx);
    const selectedUtxos = state.selectedUtxos;
    yield* Effect.logDebug(`[Selection] No selection needed: ${selectedUtxos.length} UTxO(s) already available from explicit inputs (collectFrom), ` + `Total lovelace: ${CoreAssets.lovelaceOf(inputAssets)}`);
  } else {
    if (isReselection) {
      yield* Effect.logDebug(`[Selection] Reselection attempt ${buildCtx.attempt + 1}: ` + `Need ${buildCtx.shortfall} more lovelace for change minUTxO`);
  ...
  if (stateAfterSelection.selectedUtxos.length === 0) {
    ...
    if (selectableUtxos.length === 0) {
      return yield* Effect.fail(new TransactionBuilderError({
        message: "Cannot build transaction: no UTxOs available and no explicit inputs provided. " + "Cardano protocol requires at least one input to prevent transaction replay."
      }));
    }
    // Select smallest UTxO to minimize excess
    const smallestUtxo = selectableUtxos.reduce((min, utxo) => CoreAssets.lovelaceOf(utxo.assets) < CoreAssets.lovelaceOf(min.assets) ? utxo : min);
    yield* Effect.logDebug(`[Selection] Enforcing minimum 1 input: selected smallest UTxO with ${CoreAssets.lovelaceOf(smallestUtxo.assets)} lovelace ` + `(Cardano protocol requirement for replay protection)`);
