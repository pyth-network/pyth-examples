 ...
 * @category helpers
 */
const sortCollateralCandidates = utxos => {
 ...
 * @category phases
 */
export const executeCollateral = () => Effect.gen(function* () {
  // Get contexts
  ...
  const targetCollateral = buildOptions.setCollateral ?? 5000000n;
  // ═══════════════════════════════════════════════════════════
  ...
  const candidates = availableUtxos.filter(utxo => {
  ...
  if (candidates.length === 0) {
    return yield* Effect.fail(new TransactionBuilderError({
      message: "No suitable UTxOs available for collateral. " + "All available UTxOs are either already selected or have reference scripts."
    }));
  }
  ...
  let totalAssets = CoreAssets.zero;
  for (const utxo of sorted) {
    ...
    yield* Effect.logDebug(`[Collateral] Selected UTxO: ${UTxO.toOutRefString(utxo)} (${CoreAssets.lovelaceOf(utxo.assets)} lovelace)`);
  ...
  yield* Effect.logDebug(`[Collateral] Selected ${selectedCollateral.length} inputs totaling ${totalLovelace} lovelace`);
  // ═══════════════════════════════════════════════════════════
  ...
  const minUtxo = yield* calculateMinimumUtxoLovelace({
    ...
    assets: returnAssets,
    datum: undefined,
    // No datum for collateral return
    scriptRef: undefined,
    // No script reference
  ...
  if (returnLovelace < minUtxo) {
    return yield* Effect.fail(new TransactionBuilderError({
      message: `Collateral return (${returnLovelace} lovelace) is below minimum UTxO requirement (${minUtxo} lovelace). ` + `This can happen when collateral inputs have many tokens. ` + `Consider selecting UTxOs with pure ADA for collateral, or provide more collateral.`
    }));
  }
