 ...
 */
const withRateLimit = effect => Effect.delay(effect, "100 millis");
// ============================================================================
// Helper Functions
// ============================================================================
/**
 ...
 */
const wrapError = operation => cause => Effect.fail(new ProviderError({
 ...
 */
const getAddressPath = addressOrCredential => {
...
const toBlockfrostValue = assets => {
  const value = {
...
const toBlockfrostDatum = datumOption => {
  if (!datumOption) {
    return {};
  }
  if (datumOption._tag === "DatumHash") {
    return {
      datumHash: Bytes.toHex(datumOption.hash)
    };
  }
  return {
    datum: PlutusData.toCBORHex(datumOption.data)
  };
};
const toBlockfrostScript = script => {
  if (!script) {
    return undefined;
  }
  switch (script._tag) {
    case "NativeScript":
      return {
        native: NativeScripts.toJSON(script.script)
      };
    case "PlutusV1":
      return {
        "plutus:v1": Bytes.toHex(script.bytes)
      };
    case "PlutusV2":
      return {
        "plutus:v2": Bytes.toHex(script.bytes)
      };
    case "PlutusV3":
      return {
        "plutus:v3": Bytes.toHex(script.bytes)
      };
  }
};
const toBlockfrostAdditionalUtxoSet = additionalUTxOs => additionalUTxOs.map(utxo => {
  const txOut = {
 ...
 */
const BlockfrostScriptInfo = /*#__PURE__*/Schema.Struct({
  script_hash: Schema.String,
  type: Schema.String,
  serialised_size: /*#__PURE__*/Schema.optional(Schema.Number)
});
/**
 * Blockfrost script CBOR response schema
 */
const BlockfrostScriptCbor = /*#__PURE__*/Schema.Struct({
  cbor: Schema.String
});
/**
 * Fetch script by hash and return as Script type
 */
const getScriptByHash = (baseUrl, projectId) => scriptHash => {
  // First get script info to determine type
  return withRateLimit(HttpUtils.get(`${baseUrl}/scripts/${scriptHash}`, BlockfrostScriptInfo, createHeaders(projectId))).pipe(Effect.catchAll(wrapError("getScriptByHash")), Effect.flatMap(info => {
    // Fetch CBOR for all script types (Blockfrost serves CBOR for native/timelock too)
    return withRateLimit(HttpUtils.get(`${baseUrl}/scripts/${scriptHash}/cbor`, BlockfrostScriptCbor, createHeaders(projectId))).pipe(Effect.catchAll(wrapError("getScriptByHash")), Effect.map(cbor => {
 ...
 */
const BlockfrostDatumCbor = /*#__PURE__*/Schema.Struct({
  cbor: Schema.String
});
/**
 ...
 */
const getDatumByHash = (baseUrl, projectId) => datumHash => {
  return withRateLimit(HttpUtils.get(`${baseUrl}/scripts/datum/${datumHash}/cbor`, BlockfrostDatumCbor, createHeaders(projectId))).pipe(Effect.map(datum => {
 ...
 */
export const getProtocolParameters = (baseUrl, projectId) => withRateLimit(HttpUtils.get(`${baseUrl}/epochs/latest/parameters`, Blockfrost.BlockfrostProtocolParameters, createHeaders(projectId)).pipe(Effect.map(Blockfrost.transformProtocolParameters), Effect.catchAll(wrapError("getProtocolParameters"))));
/**
 ...
 */
export const getUtxos = (baseUrl, projectId) => addressOrCredential => {
  ...
  const fetchPage = page => withRateLimit(HttpUtils.get(`${baseUrl}/addresses/${addressPath}/utxos?page=${page}&count=100`, Schema.Array(Blockfrost.BlockfrostUTxO), createHeaders(projectId)));
  const fetchAllPages = Effect.gen(function* () {
    ...
    while (hasMore) {
      ...
      Effect.catchIf(is404Error, () => Effect.succeed([])));
  ...
  const transformWithResolution = utxo => {
    const transactionId = TransactionHash.fromHex(utxo.tx_hash);
    const address = CoreAddress.fromBech32(utxo.address);
    const scriptEffect = utxo.reference_script_hash ? fetchScript(utxo.reference_script_hash).pipe(Effect.map(s => s), Effect.catchAll(() => Effect.succeed(undefined))) : Effect.succeed(undefined);
    const dataHash = utxo.data_hash;
    const datumEffect = utxo.inline_datum ? Effect.succeed(new InlineDatum.InlineDatum({
      data: PlutusData.fromCBORHex(utxo.inline_datum)
    })) : dataHash ? fetchDatum(dataHash).pipe(Effect.catchAll(() => Effect.succeed(DatumHash.fromHex(dataHash)))) : Effect.succeed(undefined);
    return Effect.all([scriptEffect, datumEffect]).pipe(Effect.map(([scriptRef, datumOption]) => {
  ...
  return fetchAllPages.pipe(Effect.flatMap(utxos => Effect.forEach(utxos.filter(u => u.tx_hash !== ""), transformWithResolution, {
    concurrency: 10
 ...
 */
export const getUtxosWithUnit = (baseUrl, projectId) => (addressOrCredential, unit) => {
  const addressPath = getAddressPath(addressOrCredential);
  const fetchScript = getScriptByHash(baseUrl, projectId);
  const fetchDatum = getDatumByHash(baseUrl, projectId);
  // Fetch all pages of UTxOs
  const fetchPage = page => withRateLimit(HttpUtils.get(`${baseUrl}/addresses/${addressPath}/utxos/${unit}?page=${page}&count=100`, Schema.Array(Blockfrost.BlockfrostUTxO), createHeaders(projectId)));
  const fetchAllPages = Effect.gen(function* () {
    const allUtxos = [];
    let page = 1;
    let hasMore = true;
 ...
 */
export const getUtxoByUnit = (baseUrl, projectId) => unit => {
  const fetchScript = getScriptByHash(baseUrl, projectId);
  const fetchDatum = getDatumByHash(baseUrl, projectId);
  // First, get addresses holding this unit
  return withRateLimit(HttpUtils.get(`${baseUrl}/assets/${unit}/addresses`, Schema.Array(Blockfrost.BlockfrostAssetAddress), createHeaders(projectId))).pipe(Effect.flatMap(addresses => {
    ...
    return withRateLimit(HttpUtils.get(`${baseUrl}/addresses/${address}/utxos/${unit}`, Schema.Array(Blockfrost.BlockfrostUTxO), createHeaders(projectId))).pipe(Effect.flatMap(utxos => {
      if (utxos.length === 0) {
        return Effect.fail(new ProviderError({
 ...
 */
export const getUtxosByOutRef = (baseUrl, projectId) => inputs => {
  const fetchScript = getScriptByHash(baseUrl, projectId);
  const fetchDatum = getDatumByHash(baseUrl, projectId);
  // Blockfrost doesn't have a bulk endpoint, so we need to make individual calls
  const effects = inputs.map(input => withRateLimit(HttpUtils.get(`${baseUrl}/txs/${TransactionHash.toHex(input.transactionId)}/utxos`, Blockfrost.BlockfrostTxUtxos, createHeaders(projectId))).pipe(Effect.flatMap(txUtxos => {
    ...
    return Effect.forEach(matchingOutputs, output => {
      const scriptEffect = output.reference_script_hash ? fetchScript(output.reference_script_hash).pipe(Effect.map(s => s), Effect.catchAll(() => Effect.succeed(undefined))) : Effect.succeed(undefined);
      const dataHash = output.data_hash;
      const datumEffect = output.inline_datum ? Effect.succeed(new InlineDatum.InlineDatum({
 ...
 */
export const getDelegation = (baseUrl, projectId) => rewardAddress => {
  // RewardAddress is a branded string, use it directly
  return withRateLimit(HttpUtils.get(`${baseUrl}/accounts/${rewardAddress}`, Blockfrost.BlockfrostDelegation, createHeaders(projectId))).pipe(Effect.map(Blockfrost.transformDelegation),
 ...
 */
export const getDatum = (baseUrl, projectId) => datumHash => {
  const datumHashHex = Bytes.toHex(datumHash.hash);
  return withRateLimit(HttpUtils.get(`${baseUrl}/scripts/datum/${datumHashHex}/cbor`, BlockfrostDatumCbor, createHeaders(projectId)).pipe(Effect.flatMap(datum => {
    // Parse CBOR hex to PlutusData
    return Effect.try({
      try: () => Schema.decodeSync(PlutusData.FromCBORHex())(datum.cbor),
      catch: error => new ProviderError({
        message: "Failed to parse datum CBOR",
        cause: error
      })
    });
 ...
 */
export const awaitTx = (baseUrl, projectId) => (txHash, checkInterval = 5000, timeout = 300_000) => {
  const txHashHex = TransactionHash.toHex(txHash);
  const checkTx = withRateLimit(HttpUtils.get(`${baseUrl}/txs/${txHashHex}`, Schema.Struct({
    hash: Schema.String
  }), createHeaders(projectId)).pipe(Effect.map(() => true), Effect.catchAll(wrapError("awaitTx"))));
  return Effect.retry(checkTx, Schedule.fixed(`${checkInterval} millis`)).pipe(Effect.timeout(timeout), Effect.catchAllCause(cause => Effect.fail(new ProviderError({
    cause,
    message: "Blockfrost awaitTx failed"
  }))));
};
/**
 * Submit transaction
 ...
 */
export const submitTx = (baseUrl, projectId) => tx => {
  ...
  return withRateLimit(HttpUtils.postUint8Array(`${baseUrl}/tx/submit`, cborBytes, Blockfrost.BlockfrostSubmitResponse, headers).pipe(Effect.flatMap(txHashHex => {
    // Parse transaction hash from hex string
    return Effect.try({
      try: () => Schema.decodeSync(TransactionHash.FromHex)(txHashHex),
      catch: error => new ProviderError({
        message: "Failed to parse transaction hash",
        cause: error
      })
    });
...
export const evaluateTx = (baseUrl, projectId) => (tx, additionalUTxOs) => {
  ...
  const additionalUtxoSet = additionalUTxOs && additionalUTxOs.length > 0 ? toBlockfrostAdditionalUtxoSet(additionalUTxOs) : [];
  const payload = {
    cbor: txCborHex,
    additionalUtxoSet
  };
  return withRateLimit(HttpUtils.postJson(`${baseUrl}/utils/txs/evaluate/utxos`, payload, Blockfrost.JsonwspOgmiosEvaluationResponse, headers).pipe(Effect.flatMap(Blockfrost.transformJsonwspOgmiosEvaluationResult), Effect.catchAll(wrapError("evaluateTx"))));
};
//# sourceMappingURL=BlockfrostEffect.js.map