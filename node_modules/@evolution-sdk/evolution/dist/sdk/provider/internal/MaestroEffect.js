 ...
 */
const createHeadersWithAmounts = apiKey => ({
  "api-key": apiKey,
  "User-Agent": "evolution-sdk",
 ...
 */
export const getProtocolParameters = (baseUrl, apiKey) => HttpUtils.get(`${baseUrl}/protocol-parameters`, Maestro.MaestroTimestampedResponse(Maestro.MaestroProtocolParameters), createHeaders(apiKey)).pipe(Effect.map(response => Maestro.transformProtocolParameters(response.data)), Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("get protocol parameters")));
// ============================================================================
// UTxO Queries
// ============================================================================
/**
 ...
 */
export const getUtxos = (baseUrl, apiKey) => addressOrCredential => Effect.gen(function* () {
 ...
 */
export const getUtxosWithUnit = (baseUrl, apiKey) => (addressOrCredential, unit) => Effect.gen(function* () {
 ...
 */
export const getUtxosByOutRef = (baseUrl, apiKey) => inputs => Effect.gen(function* () {
  ...
  for (const [txHash, indices] of byTxHash) {
    const tx = yield* HttpUtils.get(`${baseUrl}/transactions/${txHash}`, Maestro.MaestroTimestampedResponse(Maestro.MaestroTransaction), createHeadersWithAmounts(apiKey)).pipe(Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("get UTxOs by outRef")));
 ...
 */
export const getDelegation = (baseUrl, apiKey) => rewardAddress => HttpUtils.get(`${baseUrl}/accounts/${rewardAddress}`, Maestro.MaestroTimestampedResponse(Maestro.MaestroDelegation), createHeaders(apiKey)).pipe(Effect.map(response => Maestro.transformDelegation(response.data)), Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("get delegation")));
// ============================================================================
 ...
 */
export const submitTx = (baseUrl, apiKey, turboSubmit) => tx => Effect.gen(function* () {
  ...
  const response = yield* HttpUtils.postUint8Array(`${baseUrl}${endpoint}`, txBytes, Schema.String,
  // Expecting transaction hash as response
  createHeaders(apiKey)).pipe(Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("submit transaction")));
 ...
 */
export const evaluateTx = (baseUrl, apiKey) => (tx, additionalUTxOs) => Effect.gen(function* () {
  ...
  const additionalUtxos = additionalUTxOs?.map(utxo => {
    const txOut = new TxOut.TransactionOutput({
      ...
      scriptRef: utxo.scriptRef ? new ScriptRef.ScriptRef({
        bytes: Bytes.fromHex(Script.toCBORHex(utxo.scriptRef))
      }) : undefined
    });
    return {
  ...
  const requestBody = {
    ...
    ...(additionalUtxos && additionalUtxos.length > 0 ? {
      additional_utxos: additionalUtxos
    } : {})
  };
  const response = yield* HttpUtils.postJson(`${baseUrl}/transactions/evaluate`, requestBody, Maestro.MaestroEvalResult, createHeaders(apiKey)).pipe(Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("evaluate transaction")));
  return Maestro.transformEvaluationResult(response);
});
/**
 * Get single UTxO by unit (asset policy + name)
 */
export const getUtxoByUnit = (baseUrl, apiKey) => unit => Effect.gen(function* () {
  // Get the first UTxO reference from the asset endpoint (simplified response)
  const refs = yield* HttpUtils.get(`${baseUrl}/assets/${unit}/utxos?count=1`, Maestro.MaestroPaginatedResponse(Maestro.MaestroAssetUTxORef), createHeadersWithAmounts(apiKey)).pipe(Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("get UTxO by unit")));
 ...
 */
export const getDatum = (baseUrl, apiKey) => datumHash => Effect.gen(function* () {
  const datumHashHex = Bytes.toHex(datumHash.hash);
  const response = yield* HttpUtils.get(`${baseUrl}/datums/${datumHashHex}`, Maestro.MaestroTimestampedResponse(Schema.Struct({
    bytes: Schema.String
  })), {
    "api-key": apiKey,
 ...
 */
export const awaitTx = (baseUrl, apiKey) => (txHash, checkInterval, timeout = 160_000) => {
  const txHashHex = TransactionHash.toHex(txHash);
  return Effect.gen(function* () {
    const interval = checkInterval || 5000; // Default 5 seconds
    while (true) {
      // Check if transaction exists and is confirmed
      const result = yield* HttpUtils.get(`${baseUrl}/transactions/${txHashHex}`, Maestro.MaestroTimestampedResponse(Schema.Struct({
        tx_hash: Schema.String,
        block_hash: Schema.String
      })), createHeaders(apiKey)).pipe(Effect.timeout(TIMEOUT), Effect.catchAll(wrapError("awaitTx")), Effect.either);
  ...
  }).pipe(Effect.timeout(timeout), Effect.catchAllCause(cause => Effect.fail(new ProviderError({
    cause,
    message: "Maestro awaitTx failed"
  }))));
};
// ============================================================================
// Pagination Helpers
 ...
 */
const getUtxosWithPagination = (url, apiKey, maxCount) => Effect.gen(function* () {
  let allUtxos = [];
  let cursor = undefined;
  while (true) {
    // Build URL with cursor if available
    const requestUrl = cursor ? `${url}?cursor=${cursor}` : url;
    const page = yield* HttpUtils.get(requestUrl, Maestro.MaestroPaginatedResponse(Maestro.MaestroUTxO), createHeadersWithAmounts(apiKey) // Use amounts-as-strings for better precision
...
//# sourceMappingURL=MaestroEffect.js.map