 ...
 */
export const filterStatusOk = self => self.status >= 200 && self.status < 300 ? Effect.succeed(self) : self.text.pipe(Effect.flatMap(text => Effect.fail(new HttpClientError.ResponseError({
  response: self,
 ...
 */
export const get = (url, schema, headers) => HttpClient.get(url, headers ? {
  headers
} : undefined).pipe(Effect.flatMap(filterStatusOk), Effect.flatMap(response => response.json), Effect.flatMap(Schema.decodeUnknown(schema)), Effect.provide(FetchHttpClient.layer));
/**
 * Performs a POST request with JSON body and decodes the response using the provided schema
 */
export const postJson = (url, body, schema, headers) => Effect.gen(function* () {
 ...
 */
export const postUint8Array = (url, body, schema, headers) => Effect.gen(function* () {
  let request = HttpClientRequest.post(url);
