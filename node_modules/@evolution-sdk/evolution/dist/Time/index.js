 ...
 * ```
 */
export const slotToUnixTime = (slot, slotConfig) => {
 ...
 * ```
 */
export const unixTimeToSlot = (unixTime, slotConfig) => {
 ...
 * ```
 */
export const getCurrentSlot = network => {
 ...
 * @since 2.0.0
 */
export const isSlotInFuture = (slot, slotConfig) => {
  const currentTime = nowFn();
 ...
 * @since 2.0.0
 */
export const isSlotInPast = (slot, slotConfig) => {
  const currentTime = nowFn();
  const slotTime = slotToUnixTime(slot, slotConfig);
  return slotTime < currentTime;
};
 ...
 * ```
 */
export const getSlotAt = (offsetMs, network) => {
  const config = SLOT_CONFIG_NETWORK[network];
