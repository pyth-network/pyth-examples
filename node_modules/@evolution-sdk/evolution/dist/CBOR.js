 ...
 * @category errors
 */
export class CBORError extends /*#__PURE__*/Data.TaggedError("CBORError") {}
/**
 ...
 * @category constants
 */
export const CML_DEFAULT_OPTIONS = {
  mode: "custom",
 ...
 * @category constants
 */
export const CML_DATA_DEFAULT_OPTIONS = {
  mode: "custom",
 ...
 * @category constants
 */
export const AIKEN_DEFAULT_OPTIONS = {
  mode: "custom",
  useIndefiniteArrays: true,
  useIndefiniteMaps: true,
 ...
 * @category constants
 */
export const STRUCT_FRIENDLY_OPTIONS = {
  mode: "custom",
  useIndefiniteArrays: false,
  useIndefiniteMaps: false,
  useDefiniteForEmpty: true,
 ...
 * @category constants
 */
export const CARDANO_NODE_DATA_OPTIONS = {
  mode: "custom",
  useIndefiniteArrays: false,
  useIndefiniteMaps: false,
  useDefiniteForEmpty: true,
...
const TEXT_DECODER = /*#__PURE__*/new TextDecoder("utf-8", {
  fatal: true
});
// Shared float encoding buffers to reduce allocations in hot paths
const FLOAT32_BUF = /*#__PURE__*/new ArrayBuffer(4);
const FLOAT32_VIEW = /*#__PURE__*/new DataView(FLOAT32_BUF);
const FLOAT32_BYTES = /*#__PURE__*/new Uint8Array(FLOAT32_BUF);
const FLOAT64_BUF = /*#__PURE__*/new ArrayBuffer(8);
const FLOAT64_VIEW = /*#__PURE__*/new DataView(FLOAT64_BUF);
const FLOAT64_BYTES = /*#__PURE__*/new Uint8Array(FLOAT64_BUF);
/**
 ...
 *
 */
export const encodeArrayAsDefinite = items => {
  const len = items.length;
  // Compute header length based on array length (major type 4: arrays)
  let headerLen = 0;
  if (len < 24) headerLen = 1;else if (len < 256) headerLen = 2;else if (len < 65536) headerLen = 3;else headerLen = 5;
 ...
 *
 */
export const encodeArrayAsIndefinite = items => {
  const totalItemsLen = items.reduce((acc, b) => acc + b.length, 0);
 ...
 *
 */
export const encodeTaggedValue = (tag, valueBytes) => {
  if (tag < 0) throw new Error("CBOR.encodeTaggedValue: negative tag");
  // Compute header length based on tag value (major type 6: tags)
  let headerLen = 0;
  if (tag < 24) headerLen = 1;else if (tag < 256) headerLen = 2;else if (tag < 65536) headerLen = 3;else headerLen = 5;
...
export const isInteger = /*#__PURE__*/Schema.is(Integer);
// Byte String (Major Type 2)
export const ByteArray = Schema.Uint8ArrayFromSelf;
export const isByteArray = /*#__PURE__*/Schema.is(ByteArray);
// Text String (Major Type 3)
export const Text = Schema.String;
// Array (Major Type 4)
export const ArraySchema = /*#__PURE__*/Schema.Array(/*#__PURE__*/Schema.suspend(() => CBORSchema));
export const isArray = /*#__PURE__*/Schema.is(ArraySchema);
// Map (Major Type 5)
export const MapSchema = /*#__PURE__*/Schema.ReadonlyMapFromSelf({
  key: /*#__PURE__*/Schema.suspend(() => CBORSchema),
  value: /*#__PURE__*/Schema.suspend(() => CBORSchema)
});
export const isMap = /*#__PURE__*/Schema.is(MapSchema);
...
export const RecordSchema = /*#__PURE__*/Schema.Record({
  key: Schema.String,
  // Keep only string keys for Schema compatibility
  value: /*#__PURE__*/Schema.suspend(() => CBORSchema)
});
export const isRecord = /*#__PURE__*/Schema.is(RecordSchema);
// Tag (Major Type 6)
export const Tag = /*#__PURE__*/Schema.TaggedStruct("Tag", {
  tag: Schema.Number,
  value: /*#__PURE__*/Schema.suspend(() => CBORSchema)
});
export const tag = (tag, value) => Schema.TaggedStruct("Tag", {
  tag: Schema.Literal(tag),
  value
});
// Map function to create a schema for CBOR maps with specific key and value types
export const map = (key, value) => Schema.ReadonlyMapFromSelf({
  key,
  value
});
export const isTag = /*#__PURE__*/Schema.is(Tag);
// Simple values (Major Type 7)
export const Simple = /*#__PURE__*/Schema.Union(Schema.Boolean, Schema.Null, Schema.Undefined);
// Float (Major Type 7)
 ...
 * @category schemas
 */
export const CBORSchema = /*#__PURE__*/Schema.Union(Integer, ByteArray, Text, ArraySchema, MapSchema, RecordSchema, Tag, Simple, Float).annotations({
  identifier: "CBOR",
  description: "CBOR value schema"
});
/**
 ...
 * @category schemas
 */
const CBORValueSchema = /*#__PURE__*/Schema.declare(input => {
...
export const match = (value, patterns) => {
  ...
  if (typeof value === "object" && value !== null && !Array.isArray(value) && !(value instanceof Map) && !(value instanceof Uint8Array) && !(value instanceof Tag)) {
    return patterns.record(value);
  }
 ...
 * @category schemas
 */
export const FromBytes = options => Schema.transformOrFail(Schema.Uint8ArrayFromSelf, CBORValueSchema, {
  strict: true,
  decode: (fromA, _, ast) => E.try({
    try: () => internalDecodeSync(fromA, options),
    catch: error => new ParseResult.Type(ast, fromA, `Failed to decode CBOR value: ${error instanceof CBORError ? error.message : String(error)}`)
  }),
  encode: (toI, _, ast) => E.try({
    try: () => internalEncodeSync(toI, options),
    catch: error => new ParseResult.Type(ast, toI, `Failed to encode CBOR value: ${error instanceof CBORError ? error.message : String(error)}`)
  })
});
export const FromHex = options => Schema.compose(Schema.Uint8ArrayFromHex, FromBytes(options));
// ============================================================================
...
(function (Either) {
  ...
  Either.fromCBORBytes = (bytes, options = CML_DEFAULT_OPTIONS) => E.try({
    try: () => internalDecodeSync(bytes, options),
    catch: e => e instanceof CBORError ? e : new CBORError({
      message: String(e),
      cause: e
    })
  });
  /** Decode CBOR hex string to a CBOR value, returning Either */
  Either.fromCBORHex = (hex, options = CML_DEFAULT_OPTIONS) => E.try({
    try: () => {
  ...
  Either.toCBORBytes = (value, options = CML_DEFAULT_OPTIONS) => E.try({
  ...
  Either.toCBORHex = (value, options = CML_DEFAULT_OPTIONS) => E.try({
 ...
 * @category parsing
 */
export const fromCBORBytes = (bytes, options = CML_DEFAULT_OPTIONS) => internalDecodeSync(bytes, options);
/**
 ...
 * @category parsing
 */
export const fromCBORBytesWithFormat = bytes => internalDecodeWithFormatSync(bytes);
/**
 ...
 * @category parsing
 */
export const fromCBORHex = (hex, options = CML_DEFAULT_OPTIONS) => {
  const bytes = Bytes.fromHex(hex);
  return internalDecodeSync(bytes, options);
};
/**
 ...
 * @category parsing
 */
export const fromCBORHexWithFormat = hex => {
  const bytes = Bytes.fromHex(hex);
 ...
 * @category encoding
 */
export const toCBORBytes = (value, options = CML_DEFAULT_OPTIONS) => internalEncodeSync(value, options);
/**
 ...
 * @category encoding
 */
export const toCBORBytesWithFormat = (value, format) => internalEncodeSync(value, CML_DEFAULT_OPTIONS, format);
/**
 ...
 * @category encoding
 */
export const toCBORHex = (value, options = CML_DEFAULT_OPTIONS) => Bytes.toHex(internalEncodeSync(value, options));
/**
 ...
 * @category encoding
 */
export const toCBORHexWithFormat = (value, format) => Bytes.toHex(internalEncodeSync(value, CML_DEFAULT_OPTIONS, format));
// ============================================================================
...
const encodeIntMinimal = (majorType, value) => {
  ...
  return new Uint8Array([mt | 27, high >> 24 & 0xff, high >> 16 & 0xff, high >> 8 & 0xff, high & 0xff, low >> 24 & 0xff, low >> 16 & 0xff, low >> 8 & 0xff, low & 0xff]);
};
...
const encodeIntHeader = (majorType, value, byteSize) => {
  const mt = majorType << 5;
  if (byteSize === 0) {
...
export const internalEncodeSync = (value, options = CML_DEFAULT_OPTIONS, fmt) => {
  // Explicit fmt is the only source of format metadata.
  const resolvedFmt = fmt;
  if (typeof value === "bigint") {
  ...
  if (typeof value === "object" && value !== null && "_tag" in value && value._tag === "BoundedBytes") {
    return encodeBoundedBytesSync(value.bytes);
  }
  if (typeof value === "object" && value !== null && !Array.isArray(value) && !(value instanceof Map) && !(value instanceof Uint8Array) && !(value instanceof Tag)) {
    return encodeRecordSync(value, options, resolvedFmt);
  }
...
const encodeUintSync = (value, options, fmt) => {
  if (value < 0n) throw new CBORError({
  ...
  if (value < 24n) {
    ...
    const low = Number(value & 0xffffffffn);
    const high = Number(value >> 32n);
    return new Uint8Array([27, high >> 24 & 0xff, high >> 16 & 0xff, high >> 8 & 0xff, high & 0xff, low >> 24 & 0xff, low >> 16 & 0xff, low >> 8 & 0xff, low & 0xff]);
  }
};
const encodeNintSync = (value, options, fmt) => {
  if (value >= 0n) throw new CBORError({
  ...
  if (positiveValue < 24n) {
    ...
    return new Uint8Array([0x3a, Number(positiveValue >> 24n & 0xffn), Number(positiveValue >> 16n & 0xffn), Number(positiveValue >> 8n & 0xffn), Number(positiveValue & 0xffn)]);
  } else {
    return new Uint8Array([0x3b, Number(positiveValue >> 56n & 0xffn), Number(positiveValue >> 48n & 0xffn), Number(positiveValue >> 40n & 0xffn), Number(positiveValue >> 32n & 0xffn), Number(positiveValue >> 24n & 0xffn), Number(positiveValue >> 16n & 0xffn), Number(positiveValue >> 8n & 0xffn), Number(positiveValue & 0xffn)]);
  }
};
/** Byte-string chunk header size for a given chunk length (CBOR major type 2). */
const chunkHeaderSize = len => {
  if (len < 24) return 1;
  if (len < 256) return 2;
...
const writeChunkHeader = (buf, pos, len) => {
  if (len < 24) {
    buf[pos++] = 0x40 + len;
  } else if (len < 256) {
    buf[pos++] = 0x58;
...
const encodeBoundedBytesSync = value => {
  const length = value.length;
  if (length === 0) return new Uint8Array([0x40]);
  if (length <= BOUNDED_BYTES_CHUNK_SIZE) {
    // Definite-length encoding
    let headerLen;
    if (length < 24) headerLen = 1;else if (length < 256) headerLen = 2;else headerLen = 3;
    const out = new Uint8Array(headerLen + length);
    if (length < 24) out[0] = 0x40 + length;else if (length < 256) {
      out[0] = 0x58;
      out[1] = length;
    } else {
      out[0] = 0x59;
...
const encodeBytesSync = (value, options, fmt) => {
  const length = value.length;
  ...
  if (length < 24) {
    ...
    headerBytes = new Uint8Array([0x5a, length >> 24 & 0xff, length >> 16 & 0xff, length >> 8 & 0xff, length & 0xff]);
  } else {
    throw new CBORError({
...
export const BoundedBytes = {
  ...
  is: value => typeof value === "object" && value !== null && "_tag" in value && value._tag === "BoundedBytes"
};
const encodeTextSync = (value, options, fmt) => {
  ...
  if (length < 24) {
    ...
    headerBytes = new Uint8Array([0x7a, length >> 24 & 0xff, length >> 16 & 0xff, length >> 8 & 0xff, length & 0xff]);
  } else {
    throw new CBORError({
...
const encodeArraySync = (value, options, fmt) => {
  const length = value.length;
...
const encodeMapEntriesSync = (pairs, options, fmt) => {
  ...
  const encodeAsPairs = !mapFmt && (options.mode === "canonical" || options.mode === "custom") && options.encodeMapAsPairs === true;
  // If encoding as array of pairs (Aiken/Plutus style), delegate to array encoding
  if (encodeAsPairs) {
    const pairArrays = pairs.map(([k, v]) => [k, v]);
  ...
  if (mapFmt?.length !== undefined) {
    ...
    headerBytes = new Uint8Array([0xba, length >> 24 & 0xff, length >> 16 & 0xff, length >> 8 & 0xff, length & 0xff]);
  } else {
    throw new CBORError({
 ...
 * @category equality
 */
export const equals = /*#__PURE__*/Schema.equivalence(CBORSchema);
/**
...
const encodeMapSync = (value, options, fmt) => {
  const mapFmt = fmt?._tag === "map" ? fmt : undefined;
  ...
  if (mapFmt?.keyOrder && mapFmt.keyOrder.length > 0) {
    ...
    for (let j = 0; j < mapFmt.keyOrder.length; j++) {
      ...
      if (mapped !== undefined) {
        ...
        reorderedEntries.push(mapFmt.entries[j] ?? [{
          _tag: "simple"
        }, {
          _tag: "simple"
        }]);
...
const encodeRecordSync = (value, options, fmt) => {
...
const encodeTagSync = (tag, value, options, fmt) => {
  ...
  if (tagFmt !== undefined) {
    const header = tagFmt.width !== undefined ? encodeIntHeader(6, BigInt(tag), tagFmt.width) : (() => {
      const useMinimal = options.mode !== "custom" || options.useMinimalEncoding;
...
const SIMPLE_FALSE = /*#__PURE__*/new Uint8Array([0xf4]);
const SIMPLE_TRUE = /*#__PURE__*/new Uint8Array([0xf5]);
const SIMPLE_NULL = /*#__PURE__*/new Uint8Array([0xf6]);
const SIMPLE_UNDEFINED = /*#__PURE__*/new Uint8Array([0xf7]);
// Pre-allocated arrays for small common integers (0-23)
const SMALL_INTS = /*#__PURE__*/Array.from({
  length: 24
}, (_, i) => new Uint8Array([i]));
const encodeSimpleSync = value => {
...
const encodeFloatSync = (value, options) => {
 ...
 * @category decoding
 */
export const decodeItemWithOffset = (data, offset, options = CML_DEFAULT_OPTIONS) => decodeItemAt(data, offset, options, "none");
// Decode (sync)
export const internalDecodeSync = (data, options = DEFAULT_OPTIONS) => {
  if (data.length === 0) throw new CBORError({
    message: "Empty CBOR data"
  });
  const {
    item,
    newOffset
  } = decodeItemAt(data, 0, options, "none");
 ...
 * @category parsing
 */
export const internalDecodeWithFormatSync = data => {
  if (data.length === 0) throw new CBORError({
    message: "Empty CBOR data"
  });
...
const additionalInfoToByteSize = ai => {
  if (ai < 24) return 0;
  if (ai === 24) return 1;
  if (ai === 25) return 2;
  if (ai === 26) return 4;
...
const bytesReadToByteSize = bytesRead => bytesRead <= 1 ? 0 : bytesRead === 2 ? 1 : bytesRead === 3 ? 2 : 4;
const decodeItemAt = (data, offset, options, track) => {
  ...
  switch (majorType) {
    ...
    default:
      throw new CBORError({
        message: `Unsupported major type: ${majorType}`
      });
  }
...
const decodeUintAt = (data, offset, track) => {
  const firstByte = data[offset];
  ...
  if (additionalInfo < 24) {
    item = BigInt(additionalInfo);
    newOffset = offset + 1;
  } else if (additionalInfo === 24) {
    if (data.length < offset + 2) throw new CBORError({
      message: "Insufficient data for 1-byte unsigned integer"
    });
    item = BigInt(data[offset + 1]);
    newOffset = offset + 2;
  } else if (additionalInfo === 25) {
    if (data.length < offset + 3) throw new CBORError({
      message: "Insufficient data for 2-byte unsigned integer"
    });
    item = BigInt(data[offset + 1]) * 256n + BigInt(data[offset + 2]);
    newOffset = offset + 3;
  } else if (additionalInfo === 26) {
    if (data.length < offset + 5) throw new CBORError({
      message: "Insufficient data for 4-byte unsigned integer"
    });
    item = BigInt(data[offset + 1]) * 16777216n + BigInt(data[offset + 2]) * 65536n + BigInt(data[offset + 3]) * 256n + BigInt(data[offset + 4]);
    newOffset = offset + 5;
  } else if (additionalInfo === 27) {
    if (data.length < offset + 9) throw new CBORError({
...
const decodeNintAt = (data, offset, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  const bs = additionalInfoToByteSize(additionalInfo);
  let item;
  let newOffset;
  if (additionalInfo < 24) {
    item = -1n - BigInt(additionalInfo);
    newOffset = offset + 1;
  } else if (additionalInfo === 24) {
    if (data.length < offset + 2) throw new CBORError({
      message: "Insufficient data for 1-byte negative integer"
    });
    item = -1n - BigInt(data[offset + 1]);
    newOffset = offset + 2;
  } else if (additionalInfo === 25) {
    if (data.length < offset + 3) throw new CBORError({
      message: "Insufficient data for 2-byte negative integer"
    });
    item = -1n - (BigInt(data[offset + 1]) * 256n + BigInt(data[offset + 2]));
    newOffset = offset + 3;
  } else if (additionalInfo === 26) {
    if (data.length < offset + 5) throw new CBORError({
      message: "Insufficient data for 4-byte negative integer"
    });
    const v = BigInt(data[offset + 1]) * 16777216n + BigInt(data[offset + 2]) * 65536n + BigInt(data[offset + 3]) * 256n + BigInt(data[offset + 4]);
    item = -1n - v;
    newOffset = offset + 5;
  } else if (additionalInfo === 27) {
    if (data.length < offset + 9) throw new CBORError({
...
const decodeLengthAt = (data, offset) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo < 24) {
    return {
...
const decodeBytesAt = (data, offset, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    ...
    while (cur < data.length) {
      ...
      if (major !== CBOR_MAJOR_TYPE.BYTE_STRING) throw new CBORError({
        message: "Expected byte string chunk"
      });
      const {
        bytesRead,
        length
      } = decodeLengthAt(data, cur);
      const start = cur + bytesRead;
      const end = start + length;
      if (end > data.length) throw new CBORError({
        message: "Insufficient data for byte string chunk"
      });
      chunks.push(data.subarray(start, end));
      if (track !== "none") chunkInfos.push({
        length,
        byteSize: bytesReadToByteSize(bytesRead)
      });
      cur = end;
    }
    if (!foundBreak) {
      throw new CBORError({
        message: "Indefinite byte string missing break byte (0xff)"
      });
    }
    let total = 0;
    ...
    const {
      bytesRead,
      length
    } = decodeLengthAt(data, offset);
    const start = offset + bytesRead;
    const end = start + length;
    if (end > data.length) throw new CBORError({
      message: "Insufficient data for byte string"
    });
...
const decodeTextAt = (data, offset, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    let cur = offset + 1;
    ...
    while (cur < data.length) {
      ...
      const major = b >> 5 & 0x07;
      if (major !== CBOR_MAJOR_TYPE.TEXT_STRING) throw new CBORError({
        message: "Expected text string chunk"
      });
      const {
        bytesRead,
        length
      ...
      const end = start + length;
      if (end > data.length) throw new CBORError({
        message: "Insufficient data for text string chunk"
      });
    ...
    if (!foundBreak) {
      throw new CBORError({
        message: "Indefinite text string missing break byte (0xff)"
      });
    }
    ...
    const end = start + length;
    if (end > data.length) throw new CBORError({
      message: "Insufficient data for text string"
    });
...
const decodeArrayAt = (data, offset, options, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    const arr = [];
    ...
    if (track === "none") {
      ...
      if (!foundBreak) throw new CBORError({
        message: "Indefinite array missing break byte (0xff)"
      });
      return {
        item: arr,
        newOffset: cur
    ...
    while (cur < data.length) {
      ...
      arr.push(child.item);
      childFormats.push(child.format);
      cur = child.newOffset;
    }
    if (!foundBreak) throw new CBORError({
      message: "Indefinite array missing break byte (0xff)"
    ...
    if (track === "none") {
      for (let i = 0; i < length; i++) {
        const {
          item,
          newOffset
        } = decodeItemAt(data, cur, options, "none");
        arr[i] = item;
        cur = newOffset;
      }
      return {
    ...
    const childFormats = new Array(length);
    for (let i = 0; i < length; i++) {
      const child = decodeItemAt(data, cur, options, "format");
      arr[i] = child.item;
      childFormats[i] = child.format;
      cur = child.newOffset;
    }
    return {
      item: arr,
...
const decodeMapAt = (data, offset, options, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  const isObj = options.mode === "custom" && options.mapsAsObjects;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    const map = isObj ? {} : new Map();
    let cur = offset + 1;
    let foundBreak = false;
    if (track === "none") {
      while (cur < data.length) {
        ...
        cur = v.newOffset;
        if (map instanceof Map) map.set(k.item, v.item);else map[String(k.item)] = v.item;
      }
      if (!foundBreak) throw new CBORError({
        message: "Indefinite map missing break byte (0xff)"
      });
      return {
        item: map,
        newOffset: cur
    ...
    while (cur < data.length) {
      ...
      cur = v.newOffset;
      keyOrderBytes.push(data.slice(keyStart, k.newOffset));
      entryFormats.push([k.format, v.format]);
      if (map instanceof Map) map.set(k.item, v.item);else map[String(k.item)] = v.item;
    }
    if (!foundBreak) throw new CBORError({
      message: "Indefinite map missing break byte (0xff)"
    ...
    return {
      ...
      format: {
        ...
        keyOrder: keyOrderBytes,
        entries: entryFormats
      }
    };
  } else {
    const {
    ...
    const keyOrderBytes = new Array(length);
    const entryFormats = new Array(length);
    for (let i = 0; i < length; i++) {
      const keyStart = cur;
      const k = decodeItemAt(data, cur, options, "format");
      cur = k.newOffset;
...
const decodeTagAt = (data, offset, options, track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  let tagValue;
  let cur = offset;
  ...
  if (tagValue === 2 || tagValue === 3) {
    if (!(inner.item instanceof Uint8Array)) throw new CBORError({
      message: `Expected bytes for bigint tag ${tagValue}`
    });
    let n = 0n;
...
const decodeSimpleOrFloatAt = (data, offset, _track) => {
  const firstByte = data[offset];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_SIMPLE.FALSE) return {
    item: false,
    newOffset: offset + 1
  };
  if (additionalInfo === CBOR_SIMPLE.TRUE) return {
    item: true,
    newOffset: offset + 1
  };
  if (additionalInfo === CBOR_SIMPLE.NULL) return {
    item: null,
    newOffset: offset + 1
  };
  if (additionalInfo === CBOR_SIMPLE.UNDEFINED) return {
    item: undefined,
    newOffset: offset + 1
  };
  if (additionalInfo < 24) {
...
const decodeBytesWithLengthSync = data => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    let offset = 1;
    ...
    while (offset < data.length) {
      ...
      if (chunkMajorType !== CBOR_MAJOR_TYPE.BYTE_STRING) {
        throw new CBORError({
          message: `Invalid chunk in indefinite byte string: major type ${chunkMajorType}`
        });
      }
      const {
        bytesRead,
        length: chunkLength
      } = decodeLengthSync(data, offset);
      offset += bytesRead;
      if (data.length < offset + chunkLength) throw new CBORError({
        message: "Insufficient data for byte string chunk"
      });
    ...
    if (!foundBreak) throw new CBORError({
      message: "Missing break in indefinite-length byte string"
    });
    ...
    const {
      bytesRead,
      length
    } = decodeLengthSync(data, 0);
    if (data.length < bytesRead + length) throw new CBORError({
      message: "Insufficient data for byte string"
    });
...
const decodeTextWithLengthSync = data => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    let offset = 1;
    ...
    while (offset < data.length) {
      ...
      if (chunkMajorType !== CBOR_MAJOR_TYPE.TEXT_STRING) {
        throw new CBORError({
          message: `Invalid chunk in indefinite text string: major type ${chunkMajorType}`
        });
      }
      const {
        bytesRead,
    ...
    if (!foundBreak) throw new CBORError({
      message: "Missing break in indefinite-length text string"
    });
    return {
      item: parts.join(""),
      bytesConsumed: offset
...
const decodeItemWithLengthSync = (data, options) => {
  const firstByte = data[0];
  const majorType = firstByte >> 5 & 0x07;
  let item;
  let bytesConsumed;
    ...
    case CBOR_MAJOR_TYPE.UNSIGNED_INTEGER:
      {
        ...
        const additionalInfo = firstByte & 0x1f;
        bytesConsumed = additionalInfo < 24 ? 1 : additionalInfo === 24 ? 2 : additionalInfo === 25 ? 3 : additionalInfo === 26 ? 5 : 9;
        break;
      }
    case CBOR_MAJOR_TYPE.NEGATIVE_INTEGER:
      {
    ...
    case CBOR_MAJOR_TYPE.BYTE_STRING:
      {
        const {
          bytesConsumed: b,
          item: it
        } = decodeBytesWithLengthSync(data);
        item = it;
        bytesConsumed = b;
        break;
      }
    ...
    case CBOR_MAJOR_TYPE.ARRAY:
      {
        const {
          bytesConsumed: b,
          item: it
        } = decodeArrayWithLengthSync(data, options);
        item = it;
        bytesConsumed = b;
        break;
      }
    case CBOR_MAJOR_TYPE.MAP:
      {
        const {
          bytesConsumed: b,
          item: it
        } = decodeMapWithLengthSync(data, options);
        item = it;
        bytesConsumed = b;
        break;
      }
    case CBOR_MAJOR_TYPE.TAG:
      {
        const {
          bytesConsumed: b,
          item: it
        } = decodeTagWithLengthSync(data, options);
        item = it;
        bytesConsumed = b;
        break;
      }
...
const decodeArrayWithLengthSync = (data, options) => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    const result = [];
    ...
    while (offset < data.length) {
      ...
      const {
        bytesConsumed,
        item
      } = decodeItemWithLengthSync(data.slice(offset), options);
      result.push(item);
...
const decodeMapWithLengthSync = (data, options) => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo === CBOR_ADDITIONAL_INFO.INDEFINITE) {
    const result = options.mode === "custom" && options.mapsAsObjects ? {} : new Map();
    let offset = 1;
    while (offset < data.length) {
      if (data[offset] === 0xff) {
        offset++;
      ...
      const {
        bytesConsumed: keyBytes,
        item: key
      } = decodeItemWithLengthSync(data.slice(offset), options);
      offset += keyBytes;
      const {
        bytesConsumed: valueBytes,
        item: value
      } = decodeItemWithLengthSync(data.slice(offset), options);
      offset += valueBytes;
...
const decodeTagWithLengthSync = (data, options) => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  let tag;
  let dataOffset;
  if (additionalInfo < 24) {
    ...
    throw new CBORError({
      message: `Unsupported additional info for tag: ${additionalInfo}`
    });
  }
  const {
    bytesConsumed,
    item: innerValue
  } = decodeItemWithLengthSync(data.slice(dataOffset), options);
  if (tag === 2 || tag === 3) {
    if (!(innerValue instanceof Uint8Array)) throw new CBORError({
      message: `Invalid value for bigint tag ${tag}`
    });
    const bigintValue = (() => {
      let result = 0n;
...
const decodeSimpleOrFloatSync = data => {
  const firstByte = data[0];
  const additionalInfo = firstByte & 0x1f;
  if (additionalInfo < 20) {
    ...
    if (data.length < 2) throw new CBORError({
      message: "Insufficient data for simple value (one byte)"
    });
  ...
  throw new CBORError({
    message: `Unsupported additional info for simple/float: ${additionalInfo}`
  });
};
const decodeLengthSync = (data, offset) => {
  const firstByte = data[offset];
  const majorType = firstByte >> 5 & 0x07;
  const additionalInfo = firstByte & 0x1f;
  let length = 0;
  let bytesRead = 0;
  if (majorType !== CBOR_MAJOR_TYPE.BYTE_STRING && majorType !== CBOR_MAJOR_TYPE.TEXT_STRING && majorType !== CBOR_MAJOR_TYPE.ARRAY && majorType !== CBOR_MAJOR_TYPE.MAP) {
    throw new CBORError({
      message: `Invalid major type for length decoding: ${majorType}`
    });
  }
  if (additionalInfo < 24) {
    length = additionalInfo;
    ...
    throw new CBORError({
      message: `Unsupported additional info for length: ${additionalInfo}`
    });
  }
  return {
    length,
