import { Equal, FastCheck, Hash, Inspectable, Schema } from "effect";
import * as CBOR from "./CBOR.js";
/**
 * Plutus V2 script wrapper (raw bytes).
 *
 * @since 2.0.0
 * @category model
 */
export class PlutusV2 extends /*#__PURE__*/Schema.TaggedClass("PlutusV2")("PlutusV2", {
  bytes: Schema.Uint8ArrayFromHex
}) {
  /**
   * Convert to JSON representation.
   *
   * @since 2.0.0
   * @category conversions
   */
  toJSON() {
    return {
      _tag: "PlutusV2",
      bytes: this.bytes
    };
  }
  /**
   * Convert to string representation.
   *
   * @since 2.0.0
   * @category conversions
   */
  toString() {
    return Inspectable.format(this.toJSON());
  }
  /**
   * Custom inspect for Node.js REPL.
   *
   * @since 2.0.0
   * @category conversions
   */
  [Inspectable.NodeInspectSymbol]() {
    return this.toJSON();
  }
  /**
   * Structural equality check.
   *
   * @since 2.0.0
   * @category equality
   */
  [Equal.symbol](that) {
    if (!(that instanceof PlutusV2)) return false;
    // Compare Uint8Array content byte by byte
    if (this.bytes.length !== that.bytes.length) return false;
    for (let i = 0; i < this.bytes.length; i++) {
      if (this.bytes[i] !== that.bytes[i]) return false;
    }
    return true;
  }
  /**
   * Hash code generation.
   *
   * @since 2.0.0
   * @category hashing
   */
  [Hash.symbol]() {
    // Hash Uint8Array content byte by byte
    let h = Hash.hash(this.bytes.length);
    for (const byte of this.bytes) {
      h = Hash.combine(h)(Hash.hash(byte));
    }
    return Hash.cached(this, h);
  }
}
/**
 * CDDL schema for PlutusV2 scripts as raw bytes.
 *
 * @since 2.0.0
 * @category schemas
 */
export const CDDLSchema = CBOR.ByteArray;
/**
 * CDDL transformation schema for PlutusV2.
 *
 * @since 2.0.0
 * @category schemas
 */
export const FromCDDL = /*#__PURE__*/Schema.transform(CDDLSchema, /*#__PURE__*/Schema.typeSchema(PlutusV2), {
  strict: true,
  encode: toI => toI.bytes,
  decode: fromA => new PlutusV2({
    bytes: fromA
  })
});
/**
 * FastCheck arbitrary for PlutusV2.
 *
 * @since 2.0.0
 * @category arbitrary
 */
export const arbitrary = /*#__PURE__*/FastCheck.uint8Array({
  minLength: 1,
  maxLength: 512
}).map(script => new PlutusV2({
  bytes: script
}));
//# sourceMappingURL=PlutusV2.js.map