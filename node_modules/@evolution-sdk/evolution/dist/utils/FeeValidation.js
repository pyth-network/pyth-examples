 ...
 * @category validation
 */
export const validateTransactionFee = (transaction, protocolParams, fakeWitnessSet) => {
  ...
  const txToMeasure = fakeWitnessSet ? new Transaction.Transaction({
 ...
 * @category validation
 */
export const assertValidFee = (transaction, protocolParams, fakeWitnessSet) => {
  const result = validateTransactionFee(transaction, protocolParams, fakeWitnessSet);
  if (!result.isValid) {
    throw new Error(`Transaction fee is invalid. ` + `Actual: ${result.actualFee} lovelace, ` + `Minimum required: ${result.minRequiredFee} lovelace, ` + `Underpayment: ${-result.difference} lovelace ` + `(Transaction size: ${result.txSizeBytes} bytes)`);
  }
};
//# sourceMappingURL=FeeValidation.js.map