 ...
 */
export const hashTransaction = body => {
 ...
 */
export const hashTransactionRaw = bodyBytes => {
  const digest = blake2b(bodyBytes, {
    dkLen: 32
  });
  return new TransactionHash.TransactionHash({
    hash: digest
 ...
 */
const encodeDatumsTaggedSet = (datums, options = CBOR.CML_DATA_DEFAULT_OPTIONS) => {
 ...
 * ```
 */
export const hashScriptData = (redeemers, costModels, datums, options = CBOR.CML_DEFAULT_OPTIONS) => {
  ...
  if (hasDatums && redeemers.size === 0) {
    ...
    payload = concatBytes(new Uint8Array([0xa0]),
    // Empty map
    datumsBytes, new Uint8Array([0xa0]) // Empty map
    );
  } else {
    // Encode redeemers based on concrete type
    const redeemersBytes = redeemers._tag === "RedeemerMap" ? Redeemers.toCBORBytesMap(redeemers, options) : Redeemers.toCBORBytes(redeemers, options);
    const datumsBytes = hasDatums ? encodeDatumsTaggedSet(datums) : undefined;
    payload = datumsBytes ? concatBytes(redeemersBytes, datumsBytes, langViewsBytes) : concatBytes(redeemersBytes, langViewsBytes);
  }
  const digest = blake2b(payload, {
    dkLen: 32
  });
  return new ScriptDataHash.ScriptDataHash({
    hash: digest
  });
};
/**
 ...
 */
export const hashAuxiliaryData = aux => {
  const bytes = AuxiliaryData.toCBORBytes(aux);
  const digest = blake2b(bytes, {
    dkLen: 32
  });
  return new AuxiliaryDataHash.AuxiliaryDataHash({
    bytes: digest
  });
};
/**
 ...
 */
export const computeTotalExUnits = redeemers => {
  let mem = 0n;
  let steps = 0n;
