 ...
 * ```
 */
export class BootstrapWitness extends /*#__PURE__*/Schema.Class("BootstrapWitness")({
  publicKey: VKey.VKey,
   ...
   */
  [Equal.symbol](that) {
    return that instanceof BootstrapWitness && Equal.equals(this.publicKey, that.publicKey) && Equal.equals(this.signature, that.signature) && uint8ArrayEquals(this.chainCode, that.chainCode) && uint8ArrayEquals(this.attributes, that.attributes);
  }
  /**
 ...
 */
export const FromCDDL = /*#__PURE__*/Schema.transformOrFail(CDDLSchema, Schema.typeSchema(BootstrapWitness), {
  strict: true,
  encode: bw => Eff.gen(function* () {
    ...
    return [publicKeyBytes, signatureBytes, bw.chainCode, attributesBytes];
  }),
  decode: ([publicKeyBytes, signatureBytes, chainCode, attributes]) => Eff.gen(function* () {
  ...
  identifier: "BootstrapWitness.FromCDDL"
});
/**
 ...
 * @category encoding
 */
export const toCBORBytes = (witness, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORBytes(options))(witness);
/**
 ...
 * @category encoding
 */
export const toCBORHex = (witness, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORHex(options))(witness);
/**
 * Arbitrary generator for BootstrapWitness instances.
 */
export const arbitrary = /*#__PURE__*/FastCheck.record({
  attributes: FastCheck.oneof(FastCheck.constant(new Uint8Array([0xa0])), FastCheck.uint8Array({
    minLength: 1,
    maxLength: 64
  }).map(path => {
    const m = new Map();
  ...
  chainCode: FastCheck.uint8Array({
    minLength: 32,
    maxLength: 32
  }),
