 ...
 * @category API
 */
export const signData = (addressHex, payload, privateKey) => {
  // Create headers with algorithm and address in protected headers
  const protectedHeaders = headerMapNew().setAlgorithmId(AlgorithmId.EdDSA).setHeader(labelFromText("address"), Bytes.fromHex(addressHex));
  ...
  const ed25519Key = new EdDSA25519Key({
 ...
 * @category API
 */
export const verifyData = (addressHex, keyHash, payload, signedMessage) => {
  try {
