 ...
 * @category Model
 */
export class COSESign1 extends /*#__PURE__*/Schema.Class("COSESign1")({
  headers: /*#__PURE__*/Schema.instanceOf(Headers),
  payload: /*#__PURE__*/Schema.UndefinedOr(Schema.Uint8ArrayFromSelf),
  signature: /*#__PURE__*/Schema.instanceOf(Ed25519Signature.Ed25519Signature)
}) {
  toJSON() {
    return {
      _tag: "COSESign1",
  ...
  [Equal.symbol](that) {
    return that instanceof COSESign1 && Equal.equals(this.headers, that.headers) && Equal.equals(this.payload, that.payload) && Equal.equals(this.signature, that.signature);
  }
  [Hash.symbol]() {
   ...
   * @category Accessors
   */
  signedData(externalAad = new Uint8Array(), externalPayload) {
    // Encode protected headers to CBOR
    const protectedCbor = this.headers.protected.headers.size === 0 ? new Map() : new Map(Array.from(this.headers.protected.headers.entries()).map(([label, value]) => [label.value, value]));
    ...
    const payloadToUse = externalPayload !== undefined ? externalPayload : this.payload !== undefined ? this.payload : new Uint8Array();
    // Create Sig_structure: ["Signature1", protected, external_aad, payload]
    const sigStructure = ["Signature1", protectedBytes, externalAad, payloadToUse];
   ...
   * @category Conversion
   */
  static fromUserFacingEncoding(encoded) {
    ...
    if (expectedChecksum !== computedChecksum) {
      throw new Error(`Checksum does not match body. shown: ${expectedChecksum}, computed from body: ${computedChecksum}`);
    }
 ...
 * @category Schemas
 */
export const COSESign1FromCBORBytes = (options = CBOR.CML_DEFAULT_OPTIONS) => Schema.transformOrFail(CBOR.FromBytes(options), Schema.typeSchema(COSESign1), {
  strict: true,
  decode: (cbor, _, ast) => {
    ...
    return ParseResult.succeed(new COSESign1({
      headers,
      payload,
      signature
    }, {
 ...
 * @category Schemas
 */
export const COSESign1FromCBORHex = (options = CBOR.CML_DEFAULT_OPTIONS) => Schema.compose(Schema.Uint8ArrayFromHex, COSESign1FromCBORBytes(options)).annotations({
 ...
 * @category Model
 */
export class COSESign1Builder extends /*#__PURE__*/Schema.Class("COSESign1Builder")({
  ...
  [Equal.symbol](that) {
    return that instanceof COSESign1Builder && Bytes.equals(this.externalAad, that.externalAad) && this.hashPayload === that.hashPayload && Equal.equals(this.headers, that.headers) && this.isPayloadExternal === that.isPayloadExternal && Bytes.equals(this.payload, that.payload);
  }
  [Hash.symbol]() {
    return Hash.combine(Hash.combine(Hash.combine(Hash.combine(Hash.array(Array.from(this.externalAad)))(Hash.hash(this.hashPayload)))(Hash.hash(this.headers)))(Hash.hash(this.isPayloadExternal)))(Hash.array(Array.from(this.payload)));
  }
  /**
  ...
  hashPayloadWith224() {
    ...
    const hashedPayload = blake2b(this.payload, {
      dkLen: 28
    });
  ...
  makeDataToSign() {
    ...
    const protectedCbor = new Map(Array.from(this.headers.protected.headers.entries()).map(([label, value]) => [label.value, value]));
    const protectedBytes = Schema.encodeSync(CBOR.FromBytes(CBOR.CML_DEFAULT_OPTIONS))(protectedCbor);
    // Hash payload if needed
    const payloadToSign = this.hashPayload ? blake2b(this.payload, {
      dkLen: 28
    }) : this.payload;
    // Create Sig_structure: ["Signature1", protected, external_aad, payload]
    const sigStructure = ["Signature1", protectedBytes, this.externalAad, payloadToSign];
    return Schema.encodeSync(CBOR.FromBytes(CBOR.CML_DEFAULT_OPTIONS))(sigStructure);
  }
  /**
 ...
 */
export const coseSign1BuilderNew = (headers, payload, isPayloadExternal) => new COSESign1Builder({
 ...
 * @category Conversion
 */
export const coseSign1FromCBORBytes = (bytes, options) => Schema.decodeSync(COSESign1FromCBORBytes(options))(bytes);
/**
 ...
 * @category Conversion
 */
export const coseSign1FromCBORHex = (hex, options) => Schema.decodeSync(COSESign1FromCBORHex(options))(hex);
/**
 ...
 * @category Conversion
 */
export const coseSign1ToCBORBytes = (coseSign1, options) => Schema.encodeSync(COSESign1FromCBORBytes(options))(coseSign1);
/**
 ...
 * @category Conversion
 */
export const coseSign1ToCBORHex = (coseSign1, options) => Schema.encodeSync(COSESign1FromCBORHex(options))(coseSign1);
//# sourceMappingURL=CoseSign1.js.map