 ...
 * @category schemas
 */
export const AssetMap = /*#__PURE__*/Schema.Map({
 ...
 * @category model
 */
export class Mint extends /*#__PURE__*/Schema.Class("Mint")({
  map: /*#__PURE__*/Schema.Map({
 ...
 * @category predicates
 */
export const is = /*#__PURE__*/Schema.is(Mint);
/**
 * Create empty Mint.
 *
 * @since 2.0.0
 ...
 * @category constructors
 */
export const singleton = (policyId, assetName, amount) => {
  const assetMap = new Map([[assetName, amount]]);
  return new Mint({
    map: new Map([[policyId, assetMap]])
  });
};
/**
 ...
 */
export const insert = (mint, policyId, assetName, amount) => {
  // Find existing policy by content equality
  let existingPolicyKey;
  let existingAssetMap;
 ...
 * @category transformation
 */
export const removePolicy = (mint, policyId) => {
  // Find existing policy by content equality
  let existingPolicyKey;
 ...
 * @category transformation
 */
export const removeAsset = (mint, policyId, assetName) => {
  // Find existing policy by content equality
  let existingPolicyKey;
  let existingAssetMap;
  for (const [key, value] of mint.map.entries()) {
 ...
 * @category transformation
 */
export const get = (mint, policyId, assetName) => {
 ...
 * @category predicates
 */
export const has = (mint, policyId, assetName) => get(mint, policyId, assetName) !== undefined;
/**
 ...
 * @category lookup
 */
export const getByHex = (mint, policyIdHex, assetNameHex) => {
 ...
 * @category lookup
 */
export const getAssetsByPolicyHex = (mint, policyIdHex) => {
  const policyId = PolicyId.fromHex(policyIdHex);
  for (const [key, value] of mint.map.entries()) {
    if (Equal.equals(key, policyId)) {
      return value;
 ...
 * @category predicates
 */
export const isEmpty = mint => mint.map.size === 0;
/**
 ...
 * @category transformation
 */
export const policyCount = mint => mint.map.size;
export const CDDLSchema = /*#__PURE__*/Schema.MapFromSelf({
  key: CBOR.ByteArray,
  // Policy ID as 28-byte Uint8Array
  value: /*#__PURE__*/Schema.MapFromSelf({
    key: CBOR.ByteArray,
    // Asset name as Uint8Array (variable length)
 ...
 * @category schemas
 */
export const FromCDDL = /*#__PURE__*/Schema.transformOrFail(/*#__PURE__*/Schema.encodedSchema(CDDLSchema), /*#__PURE__*/Schema.typeSchema(Mint), {
  strict: true,
 ...
 * @category arbitrary
 */
export const arbitrary = /*#__PURE__*/FastCheck.oneof(
/*#__PURE__*/
// Sometimes generate an empty mint map
FastCheck.constant(/*#__PURE__*/empty()),
/*#__PURE__*/
...
FastCheck.uniqueArray(PolicyId.arbitrary, {
  ...
  const assetsForPolicy = () => FastCheck.uniqueArray(AssetName.arbitrary, {
    minLength: 1,
    maxLength: 5,
    selector: a => Bytes.toHex(a.bytes)
  }).chain(names => FastCheck.array(NonZeroInt64.arbitrary, {
    minLength: names.length,
    maxLength: names.length
  }).map(amounts => names.map((n, i) => [n, amounts[i]])));
  return FastCheck.array(assetsForPolicy(), {
    minLength: policies.length,
    maxLength: policies.length
  }).map(assetsEntries => fromEntries(policies.map((policy, idx) => [policy, assetsEntries[idx]])));
}));
// ============================================================================
// Root Functions
// ============================================================================
 ...
 * @category encoding
 */
export const toCBORBytes = (mint, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORBytes(options))(mint);
/**
 ...
 * @category encoding
 */
export const toCBORHex = (mint, options = CBOR.CML_DEFAULT_OPTIONS) => Schema.encodeSync(FromCBORHex(options))(mint);
