...
import { chainId, capabilities, createEvmConnector, executionConstraints, fixtures, roles } from '../src/index.js';

describe('@anaconda/evm', () => {
  it('exposes a scaffold-only capability matrix', () => {
    expect(chainId).toBe('evm');
  ...
  it('simulates routes using the fixture surface', () => {
    const connector = createEvmConnector();
    ...
    expect(result.chainId).toBe('evm');
